...
Verify First AuthList Test
    ...
    Should Contain X Times    ${test_output}    ${SPACE}${SPACE}${SPACE}${SPACE}identity${SPACE}${SPACE}${SPACE}${SPACE}: MTM1M3    3
...
Verify Eighth AuthList Test
    [Tags]    functional
    Log Many    ${output.stdout}    ${output.stderr}
    Should Contain    ${output.stdout}    Test with authList authorizedUsers=user@host,user2@other, nonAuthorizedCSCs=MTM1M3,MTM2,Test identity=MTRotator
    ${start}=    Get Index From List    ${full_list}    Test with authList authorizedUsers=user@host,user2@other, nonAuthorizedCSCs=MTM1M3,MTM2,Test identity=MTRotator
    ${end}=    Get Index From List    ${full_list}    =====================================================================    start=${start}
    ${test_output}=    Get Slice From List    ${full_list}    start=${start}    end=${end}
    Log    ${test_output}
    Should Contain    ${test_output}    === [issueCommand_setAuthList] writing a command containing :
