...
Start Sender
    ...
    Write    cd ${SALWorkDir}/${subSystem}/cpp/src
    Comment    Start Sender.
    ${input}=    Write    ./sacpp_${subSystem}_${component}_send -30861 1157947203
    ${output}=    Read Until Prompt
    Log    ${output}
...
Read Logger
    [Tags]    functional
    Switch Connection    Logger
    ${output}=    Read Until    priority : 1157947203
    Log    ${output}
    Should Contain X Times    ${output}    === Event BrakeEngaged received =     1
    Should Contain    ${output}    brakeId : -30861
    Should Contain    ${output}    priority : 1157947203
