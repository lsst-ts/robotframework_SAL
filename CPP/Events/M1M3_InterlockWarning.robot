...
Start Sender
    ...
    Write    cd ${SALWorkDir}/${subSystem}/cpp/src
    Comment    Start Sender.
    ${input}=    Write    ./sacpp_${subSystem}_${component}_send 20.0677 1 1 1 1 1 1 0 1 904234296
    ${output}=    Read Until Prompt
    Log    ${output}
...
Read Logger
    [Tags]    functional
    Switch Connection    Logger
    ${output}=    Read Until    priority : 904234296
    Log    ${output}
    Should Contain X Times    ${output}    === Event InterlockWarning received =     1
    Should Contain    ${output}    Timestamp : 20.0677
    ...
    Should Contain    ${output}    priority : 904234296
