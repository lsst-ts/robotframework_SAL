...
Start Sender
    ...
    Write    cd ${SALWorkDir}/${subSystem}/cpp/src
    Comment    Start Sender.
    ${input}=    Write    ./sacpp_${subSystem}_${component}_send 1.1764 1 0 0 0 1 0 0 1 -283743132
    ${output}=    Read Until Prompt
    Log    ${output}
...
Read Logger
    [Tags]    functional
    Switch Connection    Logger
    ${output}=    Read Until    priority : -283743132
    Log    ${output}
    Should Contain X Times    ${output}    === Event InterlockWarning received =     1
    Should Contain    ${output}    Timestamp : 1.1764
    ...
    Should Contain    ${output}    priority : -283743132
