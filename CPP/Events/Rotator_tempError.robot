...
Start Sender
    ...
    Write    cd ${SALWorkDir}/${subSystem}/cpp/src
    Comment    Start Sender.
    ${input}=    Write    ./sacpp_${subSystem}_${component}_send 620636729 test 852774694 439186205
    ${output}=    Read Until Prompt
    Log    ${output}
...
Read Logger
    [Tags]    functional
    Switch Connection    Logger
    ${output}=    Read Until    priority : 439186205
    Log    ${output}
    Should Contain X Times    ${output}    === Event tempError received =     1
    Should Contain    ${output}    priority : 620636729
    Should Contain    ${output}    device : test
    Should Contain    ${output}    severity : 852774694
    Should Contain    ${output}    priority : 439186205
