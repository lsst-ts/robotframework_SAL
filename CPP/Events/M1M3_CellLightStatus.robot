...
Start Sender
    ...
    Write    cd ${SALWorkDir}/${subSystem}/cpp/src
    Comment    Start Sender.
    ${input}=    Write    ./sacpp_${subSystem}_${component}_send 87.3408 1 1 0 -1713968515
    ${output}=    Read Until Prompt
    Log    ${output}
...
Read Logger
    [Tags]    functional
    Switch Connection    Logger
    ${output}=    Read Until    priority : -1713968515
    Log    ${output}
    Should Contain X Times    ${output}    === Event CellLightStatus received =     1
    Should Contain    ${output}    Timestamp : 87.3408
    ...
    Should Contain    ${output}    priority : -1713968515
