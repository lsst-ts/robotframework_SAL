...
Start Sender
    ...
    Write    cd ${SALWorkDir}/${subSystem}/cpp/src
    Comment    Start Sender.
    ${input}=    Write    ./sacpp_${subSystem}_${component}_send 98.2648 0 1 0 86523857
    ${output}=    Read Until Prompt
    Log    ${output}
...
Read Logger
    [Tags]    functional
    Switch Connection    Logger
    ${output}=    Read Until    priority : 86523857
    Log    ${output}
    Should Contain X Times    ${output}    === Event CellLightStatus received =     1
    Should Contain    ${output}    Timestamp : 98.2648
    ...
    Should Contain    ${output}    priority : 86523857
