...
Start Sender
    ...
    Write    cd ${SALWorkDir}/${subSystem}/cpp/src
    Comment    Start Sender.
    ${input}=    Write    ./sacpp_${subSystem}_${component}_send 49.972 0 1 0 1 1 1 0 1 1 -104562959
    ${output}=    Read Until Prompt
    Log    ${output}
...
Read Logger
    [Tags]    functional
    Switch Connection    Logger
    ${output}=    Read Until    priority : -104562959
    Log    ${output}
    Should Contain X Times    ${output}    === Event InclinometerSensorWarning received =     1
    Should Contain    ${output}    Timestamp : 49.972
    ...
    Should Contain    ${output}    priority : -104562959
