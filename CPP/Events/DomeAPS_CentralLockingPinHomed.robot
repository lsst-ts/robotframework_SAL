...
*** Variables ***
${subSystem}    domeAPS
${component}    CentralLockingPinHomed
${timeout}    30s
...
Start Sender
    ...
    Write    cd ${SALWorkDir}/${subSystem}/cpp/src
    Comment    Start Sender.
    ${input}=    Write    ./sacpp_${subSystem}_${component}_send 1429693940
    ${output}=    Read Until Prompt
    Log    ${output}
    Should Contain X Times    ${output}    === [putSample] domeAPS::logevent_CentralLockingPinHomed writing a message containing :    1
    Should Contain    ${output}    revCode \ :
    Should Contain    ${output}    === Event CentralLockingPinHomed generated =

Read Logger
    [Tags]    functional
    Switch Connection    Logger
    ${output}=    Read Until    priority : 1429693940
    Log    ${output}
    Should Contain X Times    ${output}    === Event CentralLockingPinHomed received =     1
    Should Contain    ${output}    priority : 1429693940
