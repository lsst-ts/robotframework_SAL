...
Start Sender
    ...
    Write    cd ${SALWorkDir}/${subSystem}/cpp/src
    Comment    Start Sender.
    ${input}=    Write    ./sacpp_${subSystem}_${component}_send VOlrywMtGtKZZdqrTCzEotollQKVaZVGztnVbWXfLRGwGIRPztGXNjVVkbwgaosPWBNHyeLLSVaTllpiJSxEpMmANFktZQ -171174600
    ${output}=    Read Until Prompt
    Log    ${output}
...
Read Logger
    [Tags]    functional
    Switch Connection    Logger
    ${output}=    Read Until    priority : -171174600
    Log    ${output}
    Should Contain X Times    ${output}    === Event SubsystemError received =     1
    Should Contain    ${output}    errorCode : VOlrywMtGtKZZdqrTCzEotollQKVaZVGztnVbWXfLRGwGIRPztGXNjVVkbwgaosPWBNHyeLLSVaTllpiJSxEpMmANFktZQ
    Should Contain    ${output}    priority : -171174600
