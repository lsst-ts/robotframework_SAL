...
Start Sender
    ...
    Write    cd ${SALWorkDir}/${subSystem}/cpp/src
    Comment    Start Sender.
    ${input}=    Write    ./sacpp_${subSystem}_${component}_send 12447 1227605127
    ${output}=    Read Until Prompt
    Log    ${output}
...
Read Logger
    [Tags]    functional
    Switch Connection    Logger
    ${output}=    Read Until    priority : 1227605127
    Log    ${output}
    Should Contain X Times    ${output}    === Event LockingPinEngaged received =     1
    Should Contain    ${output}    lockingPinID : 12447
    Should Contain    ${output}    priority : 1227605127
