...
Start Sender
    ...
    Write    cd ${SALWorkDir}/${subSystem}/cpp/src
    Comment    Start Sender.
    ${input}=    Write    ./sacpp_${subSystem}_${component}_send -1404 676824706
    ${output}=    Read Until Prompt
    Log    ${output}
...
Read Logger
    [Tags]    functional
    Switch Connection    Logger
    ${output}=    Read Until    priority : 676824706
    Log    ${output}
    Should Contain X Times    ${output}    === Event LockingPinEngaged received =     1
    Should Contain    ${output}    lockingPinID : -1404
    Should Contain    ${output}    priority : 676824706
