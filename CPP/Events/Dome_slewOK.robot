*** Settings ***
Documentation    Dome_slewOK sender/logger tests.
...
Start Sender
    ...
    Write    cd ${SALWorkDir}/${subSystem}/cpp/src
    Comment    Start Sender.
    ${input}=    Write    ./sacpp_${subSystem}_${component}_send 11390    #|tee ${comOut}
    ${output}=    Read Until Prompt
    Log    ${output}
    Should Contain X Times    ${output}    === [putSample] dome::logevent_slewOK writing a message containing :    1
    Should Contain    ${output}    revCode \ :
    Should Contain    ${output}    === Event slewOK generated =
...
Read Logger
    ...
    Should Contain X Times    ${output}    === Event slewOK received =     2
    Should Contain X Times    ${output}    priority : 11390    2
