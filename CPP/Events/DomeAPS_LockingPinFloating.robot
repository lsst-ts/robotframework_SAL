...
Start Sender
    ...
    Write    cd ${SALWorkDir}/${subSystem}/cpp/src
    Comment    Start Sender.
    ${input}=    Write    ./sacpp_${subSystem}_${component}_send -14154 1609521750
    ${output}=    Read Until Prompt
    Log    ${output}
...
Read Logger
    [Tags]    functional
    Switch Connection    Logger
    ${output}=    Read Until    priority : 1609521750
    Log    ${output}
    Should Contain X Times    ${output}    === Event LockingPinFloating received =     1
    Should Contain    ${output}    lockingPinID : -14154
    Should Contain    ${output}    priority : 1609521750
