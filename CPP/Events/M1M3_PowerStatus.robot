...
Start Sender
    ...
    Write    cd ${SALWorkDir}/${subSystem}/cpp/src
    Comment    Start Sender.
    ${input}=    Write    ./sacpp_${subSystem}_${component}_send 61.6584 0 1 1 0 0 1 0 0 1 1 1 0 1 0 0 0 -2062954964
    ${output}=    Read Until Prompt
    Log    ${output}
...
Read Logger
    [Tags]    functional
    Switch Connection    Logger
    ${output}=    Read Until    priority : -2062954964
    Log    ${output}
    Should Contain X Times    ${output}    === Event PowerStatus received =     1
    Should Contain    ${output}    Timestamp : 61.6584
    ...
    Should Contain    ${output}    AuxPowerNetworkACommandedOn : 1
    ...
    Should Contain    ${output}    AuxPowerNetworkCCommandedOn : 1
    Should Contain    ${output}    AuxPowerNetworkCOutputOn : 0
    Should Contain    ${output}    AuxPowerNetworkDCommandedOn : 0
    Should Contain    ${output}    AuxPowerNetworkDOutputOn : 0
    Should Contain    ${output}    priority : -2062954964
