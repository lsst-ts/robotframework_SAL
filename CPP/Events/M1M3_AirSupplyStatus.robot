...
Start Sender
    ...
    Write    cd ${SALWorkDir}/${subSystem}/cpp/src
    Comment    Start Sender.
    ${input}=    Write    ./sacpp_${subSystem}_${component}_send 27.0884 1 1 0 0 689296555
    ${output}=    Read Until Prompt
    Log    ${output}
...
Read Logger
    [Tags]    functional
    Switch Connection    Logger
    ${output}=    Read Until    priority : 689296555
    Log    ${output}
    Should Contain X Times    ${output}    === Event AirSupplyStatus received =     1
    Should Contain    ${output}    Timestamp : 27.0884
    ...
    Should Contain    ${output}    priority : 689296555
