*** Settings ***
Documentation    Dome_crawlLost sender/logger tests.
...
Start Sender
    ...
    Write    cd ${SALWorkDir}/${subSystem}/cpp/src
    Comment    Start Sender.
    ${input}=    Write    ./sacpp_${subSystem}_${component}_send 25846    #|tee ${comOut}
    ${output}=    Read Until Prompt
    Log    ${output}
    Should Contain X Times    ${output}    === [putSample] dome::logevent_crawlLost writing a message containing :    1
    Should Contain    ${output}    revCode \ :
    Should Contain    ${output}    === Event crawlLost generated =
...
Read Logger
    ...
    Should Contain X Times    ${output}    === Event crawlLost received =     2
    Should Contain X Times    ${output}    priority : 25846    2
