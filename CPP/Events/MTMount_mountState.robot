...
Start Sender
    ...
    Write    cd ${SALWorkDir}/${subSystem}/cpp/src
    Comment    Start Sender.
    ${input}=    Write    ./sacpp_${subSystem}_${component}_send 875403566 test    #|tee ${comOut}
    ${output}=    Read Until Prompt
    Log    ${output}
...
Read Logger
    ...
    Should Contain X Times    ${output}    === Event mountState received =     2
    Should Contain X Times    ${output}    priority : 875403566    2
    Should Contain X Times    ${output}    id :    2
    Should Contain    ${output}    id : 875403566
