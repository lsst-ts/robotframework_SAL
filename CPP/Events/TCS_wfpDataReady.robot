...
Start Sender
    ...
    Write    cd ${SALWorkDir}/${subSystem}/cpp/src
    Comment    Start Sender.
    ${input}=    Write    ./sacpp_${subSystem}_${component}_send 1569789161 test test    #|tee ${comOut}
    ${output}=    Read Until Prompt
    Log    ${output}
...
Read Logger
    ...
    Should Contain X Times    ${output}    === Event wfpDataReady received =     2
    Should Contain X Times    ${output}    priority : 1569789161    2
    Should Contain X Times    ${output}    priority :    2
    Should Contain    ${output}    priority : 1569789161
    Should Contain X Times    ${output}    images_location_URI :    2
    Should Contain    ${output}    images_location_URI : test
    Should Contain X Times    ${output}    images_match_URI :    2
