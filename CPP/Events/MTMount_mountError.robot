...
Start Sender
    ...
    Write    cd ${SALWorkDir}/${subSystem}/cpp/src
    Comment    Start Sender.
    ${input}=    Write    ./sacpp_${subSystem}_${component}_send 936207816 test 157659829
    ${output}=    Read Until Prompt
    Log    ${output}
...
Read Logger
    [Tags]    functional
    Switch Connection    Logger
    ${output}=    Read Until    priority : 157659829
    Log    ${output}
    Should Contain X Times    ${output}    === Event mountError received =     1
    Should Contain    ${output}    id : 936207816
    Should Contain    ${output}    text : test
    Should Contain    ${output}    priority : 157659829
