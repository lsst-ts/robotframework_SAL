...
Start Sender
    ...
    Write    cd ${SALWorkDir}/${subSystem}/cpp/src
    Comment    Start Sender.
    ${input}=    Write    ./sacpp_${subSystem}_${component}_send 347606634 test 2075659906
    ${output}=    Read Until Prompt
    Log    ${output}
...
Read Logger
    [Tags]    functional
    Switch Connection    Logger
    ${output}=    Read Until    priority : 2075659906
    Log    ${output}
    Should Contain X Times    ${output}    === Event mountError received =     1
    Should Contain    ${output}    id : 347606634
    Should Contain    ${output}    text : test
    Should Contain    ${output}    priority : 2075659906
