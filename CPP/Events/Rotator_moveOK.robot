*** Settings ***
Documentation    Rotator_moveOK sender/logger tests.
...
Start Sender
    ...
    Write    cd ${SALWorkDir}/${subSystem}/cpp/src
    Comment    Start Sender.
    ${input}=    Write    ./sacpp_${subSystem}_${component}_send 10061    #|tee ${comOut}
    ${output}=    Read Until Prompt
    Log    ${output}
    Should Contain X Times    ${output}    === [putSample] rotator::logevent_moveOK writing a message containing :    1
    Should Contain    ${output}    revCode \ :
    Should Contain    ${output}    === Event moveOK generated =
...
Read Logger
    ...
    Should Contain X Times    ${output}    === Event moveOK received =     2
    Should Contain X Times    ${output}    priority : 10061    2
