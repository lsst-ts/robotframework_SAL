...
Start Sender
    ...
    Write    cd ${SALWorkDir}/${subSystem}/cpp/src
    Comment    Start Sender.
    ${input}=    Write    ./sacpp_${subSystem}_${component}_send 1106819227 27.7536 -1653481840
    ${output}=    Read Until Prompt
    Log    ${output}
...
Read Logger
    [Tags]    functional
    Switch Connection    Logger
    ${output}=    Read Until    priority : -1653481840
    Log    ${output}
    Should Contain X Times    ${output}    === Event SelectedGrating received =     1
    Should Contain    ${output}    gratingType : 1106819227
    Should Contain    ${output}    timestamp : 27.7536
    Should Contain    ${output}    priority : -1653481840
