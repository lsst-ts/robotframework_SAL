...
Start Sender
    ...
    Write    cd ${SALWorkDir}/${subSystem}/cpp/src
    Comment    Start Sender.
    ${input}=    Write    ./sacpp_${subSystem}_${component}_send -1200827569 30.7915 2043989638
    ${output}=    Read Until Prompt
    Log    ${output}
...
Read Logger
    [Tags]    functional
    Switch Connection    Logger
    ${output}=    Read Until    priority : 2043989638
    Log    ${output}
    Should Contain X Times    ${output}    === Event SelectedGrating received =     1
    Should Contain    ${output}    gratingType : -1200827569
    Should Contain    ${output}    timestamp : 30.7915
    Should Contain    ${output}    priority : 2043989638
