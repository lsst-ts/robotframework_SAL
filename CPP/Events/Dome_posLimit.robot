*** Settings ***
Documentation    Dome_posLimit sender/logger tests.
...
Start Sender
    ...
    Write    cd ${SALWorkDir}/${subSystem}/cpp/src
    Comment    Start Sender.
    ${input}=    Write    ./sacpp_${subSystem}_${component}_send 1980509322 test test test    #|tee ${comOut}
    ${output}=    Read Until Prompt
    Log    ${output}
    Should Contain X Times    ${output}    === [putSample] dome::logevent_posLimit writing a message containing :    1
    Should Contain    ${output}    revCode \ :
    Should Contain    ${output}    === Event posLimit generated =
...
Read Logger
    ...
    Should Contain X Times    ${output}    === Event posLimit received =     2
    Should Contain X Times    ${output}    priority : 1980509322    2
    Should Contain X Times    ${output}    priority :    2
    Should Contain    ${output}    priority : 1980509322
