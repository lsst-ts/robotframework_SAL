...
Start Sender
    ...
    Write    cd ${SALWorkDir}/${subSystem}/cpp/src
    Comment    Start Sender.
    ${input}=    Write    ./sacpp_${subSystem}_${component}_send 7052 661804336
    ${output}=    Read Until Prompt
    Log    ${output}
...
Read Logger
    [Tags]    functional
    Switch Connection    Logger
    ${output}=    Read Until    priority : 661804336
    Log    ${output}
    Should Contain X Times    ${output}    === Event LockingPinDisengaged received =     1
    Should Contain    ${output}    lockingPinID : 7052
    Should Contain    ${output}    priority : 661804336
