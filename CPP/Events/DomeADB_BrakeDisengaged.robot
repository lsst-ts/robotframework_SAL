...
Start Sender
    ...
    Write    cd ${SALWorkDir}/${subSystem}/cpp/src
    Comment    Start Sender.
    ${input}=    Write    ./sacpp_${subSystem}_${component}_send 30506 2027551482
    ${output}=    Read Until Prompt
    Log    ${output}
...
Read Logger
    [Tags]    functional
    Switch Connection    Logger
    ${output}=    Read Until    priority : 2027551482
    Log    ${output}
    Should Contain X Times    ${output}    === Event BrakeDisengaged received =     1
    Should Contain    ${output}    brakeId : 30506
    Should Contain    ${output}    priority : 2027551482
