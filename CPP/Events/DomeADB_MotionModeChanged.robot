...
Start Sender
    ...
    Write    cd ${SALWorkDir}/${subSystem}/cpp/src
    Comment    Start Sender.
    ${input}=    Write    ./sacpp_${subSystem}_${component}_send 17057 1490227412
    ${output}=    Read Until Prompt
    Log    ${output}
...
Read Logger
    [Tags]    functional
    Switch Connection    Logger
    ${output}=    Read Until    priority : 1490227412
    Log    ${output}
    Should Contain X Times    ${output}    === Event MotionModeChanged received =     1
    Should Contain    ${output}    newMode : 17057
    Should Contain    ${output}    priority : 1490227412
