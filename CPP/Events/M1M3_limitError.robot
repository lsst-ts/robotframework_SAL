...
Start Sender
    ...
    Write    cd ${SALWorkDir}/${subSystem}/cpp/src
    Comment    Start Sender.
    ${input}=    Write    ./sacpp_${subSystem}_${component}_send 1012595450 test test    #|tee ${comOut}
    ${output}=    Read Until Prompt
    Log    ${output}
...
Read Logger
    ...
    Should Contain X Times    ${output}    === Event limitError received =     2
    Should Contain X Times    ${output}    priority : 1012595450    2
    Should Contain X Times    ${output}    priority :    2
    Should Contain    ${output}    priority : 1012595450
