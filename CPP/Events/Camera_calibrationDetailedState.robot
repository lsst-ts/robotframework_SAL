...
Start Sender
    ...
    Write    cd ${SALWorkDir}/${subSystem}/cpp/src
    Comment    Start Sender.
    ${input}=    Write    ./sacpp_${subSystem}_${component}_send 1984010849 -561642561
    ${output}=    Read Until Prompt
    Log    ${output}
...
Read Logger
    [Tags]    functional
    Switch Connection    Logger
    ${output}=    Read Until    priority : -561642561
    Log    ${output}
    Should Contain X Times    ${output}    === Event calibrationDetailedState received =     1
    Should Contain    ${output}    substate : 1984010849
    Should Contain    ${output}    priority : -561642561
