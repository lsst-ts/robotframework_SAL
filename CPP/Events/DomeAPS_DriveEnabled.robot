...
Start Sender
    ...
    Write    cd ${SALWorkDir}/${subSystem}/cpp/src
    Comment    Start Sender.
    ${input}=    Write    ./sacpp_${subSystem}_${component}_send 19467 19109892
    ${output}=    Read Until Prompt
    Log    ${output}
...
Read Logger
    [Tags]    functional
    Switch Connection    Logger
    ${output}=    Read Until    priority : 19109892
    Log    ${output}
    Should Contain X Times    ${output}    === Event DriveEnabled received =     1
    Should Contain    ${output}    driveId : 19467
    Should Contain    ${output}    priority : 19109892
