...
Start Sender
    ...
    Write    cd ${SALWorkDir}/${subSystem}/cpp/src
    Comment    Start Sender.
    ${input}=    Write    ./sacpp_${subSystem}_${component}_send 24561 test 668981123
    ${output}=    Read Until Prompt
    Log    ${output}
...
Read Logger
    [Tags]    functional
    Switch Connection    Logger
    ${output}=    Read Until    priority : 668981123
    Log    ${output}
    Should Contain X Times    ${output}    === Event SubsystemError received =     1
    Should Contain    ${output}    louverID : 24561
    Should Contain    ${output}    errorCode : test
    Should Contain    ${output}    priority : 668981123
