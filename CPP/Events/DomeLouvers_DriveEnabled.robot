...
Start Sender
    ...
    Write    cd ${SALWorkDir}/${subSystem}/cpp/src
    Comment    Start Sender.
    ${input}=    Write    ./sacpp_${subSystem}_${component}_send -24490 13207 1678546398
    ${output}=    Read Until Prompt
    Log    ${output}
...
Read Logger
    [Tags]    functional
    Switch Connection    Logger
    ${output}=    Read Until    priority : 1678546398
    Log    ${output}
    Should Contain X Times    ${output}    === Event DriveEnabled received =     1
    Should Contain    ${output}    louverID : -24490
    Should Contain    ${output}    driveId : 13207
    Should Contain    ${output}    priority : 1678546398
