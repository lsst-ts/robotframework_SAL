...
Start Sender
    ...
    Write    cd ${SALWorkDir}/${subSystem}/cpp/src
    Comment    Start Sender.
    ${input}=    Write    ./sacpp_${subSystem}_${component}_send 1382263630 1153331429
    ${output}=    Read Until Prompt
    Log    ${output}
    Should Contain X Times    ${output}    === [putSample] dm::logevent_dmToBase writing a message containing :    1
    Should Contain    ${output}    revCode \ :
    Should Contain    ${output}    === Event dmToBase generated =

Read Logger
    [Tags]    functional
    Switch Connection    Logger
    ${output}=    Read Until    priority : 1153331429
    Log    ${output}
    Should Contain X Times    ${output}    === Event dmToBase received =     1
    Should Contain    ${output}    image_id : 1382263630
    Should Contain    ${output}    priority : 1153331429
