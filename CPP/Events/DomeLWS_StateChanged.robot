...
Start Sender
    ...
    Write    cd ${SALWorkDir}/${subSystem}/cpp/src
    Comment    Start Sender.
    ${input}=    Write    ./sacpp_${subSystem}_${component}_send 5068 344985535
    ${output}=    Read Until Prompt
    Log    ${output}
...
Read Logger
    [Tags]    functional
    Switch Connection    Logger
    ${output}=    Read Until    priority : 344985535
    Log    ${output}
    Should Contain X Times    ${output}    === Event StateChanged received =     1
    Should Contain    ${output}    newState : 5068
    Should Contain    ${output}    priority : 344985535
