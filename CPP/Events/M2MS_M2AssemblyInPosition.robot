...
Start Sender
    ...
    Write    cd ${SALWorkDir}/${subSystem}/cpp/src
    Comment    Start Sender.
    ${input}=    Write    ./sacpp_${subSystem}_${component}_send 1965547440 true 1613859038
    ${output}=    Read Until Prompt
    Log    ${output}
...
Read Logger
    [Tags]    functional
    Switch Connection    Logger
    ${output}=    Read Until    priority : 1613859038
    Log    ${output}
    Should Contain X Times    ${output}    === Event M2AssemblyInPosition received =     1
    Should Contain    ${output}    priority : 1965547440
    Should Contain    ${output}    inPosition : true
    Should Contain    ${output}    priority : 1613859038
