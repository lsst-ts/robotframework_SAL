*** Settings ***
Documentation    M2MS_M2SummaryState sender/logger tests.
...
Start Sender
    ...
    Write    cd ${SALWorkDir}/${subSystem}/cpp/src
    Comment    Start Sender.
    ${input}=    Write    ./sacpp_${subSystem}_${component}_send 1464861189 16693    #|tee ${comOut}
    ${output}=    Read Until Prompt
    Log    ${output}
    Should Contain X Times    ${output}    === [putSample] m2ms::logevent_M2SummaryState writing a message containing :    1
    Should Contain    ${output}    revCode \ :
    Should Contain    ${output}    === Event M2SummaryState generated =
...
Read Logger
    ...
    Should Contain X Times    ${output}    === Event M2SummaryState received =     2
    Should Contain X Times    ${output}    priority : 1464861189    2
    Should Contain X Times    ${output}    priority :    2
    Should Contain    ${output}    priority : 1464861189
    Should Contain X Times    ${output}    state :    2
    Should Contain    ${output}    state : 16693
