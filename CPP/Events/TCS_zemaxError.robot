...
Start Sender
    ...
    Write    cd ${SALWorkDir}/${subSystem}/cpp/src
    Comment    Start Sender.
    ${input}=    Write    ./sacpp_${subSystem}_${component}_send 1084174546 test    #|tee ${comOut}
    ${output}=    Read Until Prompt
    Log    ${output}
...
Read Logger
    ...
    Should Contain X Times    ${output}    === Event zemaxError received =     2
    Should Contain X Times    ${output}    priority : 1084174546    2
    Should Contain X Times    ${output}    priority :    2
    Should Contain    ${output}    priority : 1084174546
    Should Contain X Times    ${output}    details :    2
