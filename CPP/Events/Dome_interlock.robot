*** Settings ***
Documentation    Dome_interlock sender/logger tests.
...
Start Sender
    ...
    Write    cd ${SALWorkDir}/${subSystem}/cpp/src
    Comment    Start Sender.
    ${input}=    Write    ./sacpp_${subSystem}_${component}_send 1270395008 test    #|tee ${comOut}
    ${output}=    Read Until Prompt
    Log    ${output}
    Should Contain X Times    ${output}    === [putSample] dome::logevent_interlock writing a message containing :    1
...
Read Logger
    ...
    Should Contain X Times    ${output}    === Event interlock received =     2
    Should Contain X Times    ${output}    priority : 1270395008    2
    Should Contain X Times    ${output}    priority :    2
    Should Contain    ${output}    priority : 1270395008
    Should Contain X Times    ${output}    detail :    2
