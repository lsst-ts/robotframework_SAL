...
Start Sender
    ...
    Write    cd ${SALWorkDir}/${subSystem}/cpp/src
    Comment    Start Sender.
    ${input}=    Write    ./sacpp_${subSystem}_${component}_send 16947 1431841437
    ${output}=    Read Until Prompt
    Log    ${output}
...
Read Logger
    [Tags]    functional
    Switch Connection    Logger
    ${output}=    Read Until    priority : 1431841437
    Log    ${output}
    Should Contain X Times    ${output}    === Event MovementEnabled received =     1
    Should Contain    ${output}    louverID : 16947
    Should Contain    ${output}    priority : 1431841437
