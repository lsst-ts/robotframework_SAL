...
Start Sender
    ...
    Write    cd ${SALWorkDir}/${subSystem}/cpp/src
    Comment    Start Sender.
    ${input}=    Write    ./sacpp_${subSystem}_${component}_send 96.6662 0 0 0 -1543334983
    ${output}=    Read Until Prompt
    Log    ${output}
...
Read Logger
    [Tags]    functional
    Switch Connection    Logger
    ${output}=    Read Until    priority : -1543334983
    Log    ${output}
    Should Contain X Times    ${output}    === Event CellLightWarning received =     1
    Should Contain    ${output}    Timestamp : 96.6662
    ...
    Should Contain    ${output}    priority : -1543334983
