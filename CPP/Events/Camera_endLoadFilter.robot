...
Start Sender
    ...
    Write    cd ${SALWorkDir}/${subSystem}/cpp/src
    Comment    Start Sender.
    ${input}=    Write    ./sacpp_${subSystem}_${component}_send -2129458909 -1904353461
    ${output}=    Read Until Prompt
    Log    ${output}
...
Read Logger
    [Tags]    functional
    Switch Connection    Logger
    ${output}=    Read Until    priority : -1904353461
    Log    ${output}
    Should Contain X Times    ${output}    === Event endLoadFilter received =     1
    Should Contain    ${output}    priority : -2129458909
    Should Contain    ${output}    priority : -1904353461
