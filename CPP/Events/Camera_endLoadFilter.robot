*** Settings ***
Documentation    Camera_endLoadFilter communications tests.
Force Tags    cpp    
...
Start Sender
    ...
    Write    cd ${SALWorkDir}/${subSystem}/cpp/src
    Comment    Start Sender.
    ${input}=    Write    ./sacpp_${subSystem}_${component}_send 1888677499
    ${output}=    Read Until Prompt
    Log    ${output}
...
Read Logger
    [Tags]    functional
    Switch Connection    Logger
    ${output}=    Read Until    priority : 1888677499
    Log    ${output}
    Should Contain X Times    ${output}    === Event endLoadFilter received =     1
    Should Contain    ${output}    priority : 1888677499
