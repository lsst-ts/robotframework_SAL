...
Start Sender
    ...
    Write    cd ${SALWorkDir}/${subSystem}/cpp/src
    Comment    Start Sender.
    ${input}=    Write    ./sacpp_${subSystem}_${component}_send 387431681 212588766
    ${output}=    Read Until Prompt
    Log    ${output}
...
Read Logger
    [Tags]    functional
    Switch Connection    Logger
    ${output}=    Read Until    priority : 212588766
    Log    ${output}
    Should Contain X Times    ${output}    === Event endInitializeGuider received =     1
    Should Contain    ${output}    priority : 387431681
    Should Contain    ${output}    priority : 212588766
