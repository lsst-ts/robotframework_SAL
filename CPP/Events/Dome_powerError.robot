*** Settings ***
Documentation    Dome_powerError sender/logger tests.
...
Start Sender
    ...
    Write    cd ${SALWorkDir}/${subSystem}/cpp/src
    Comment    Start Sender.
    ${input}=    Write    ./sacpp_${subSystem}_${component}_send 53949519 test 641612754    #|tee ${comOut}
    ${output}=    Read Until Prompt
    Log    ${output}
    Should Contain X Times    ${output}    === [putSample] dome::logevent_powerError writing a message containing :    1
    Should Contain    ${output}    revCode \ :
    Should Contain    ${output}    === Event powerError generated =
...
Read Logger
    ...
    Should Contain X Times    ${output}    === Event powerError received =     2
    Should Contain X Times    ${output}    priority : 53949519    2
    Should Contain X Times    ${output}    priority :    2
    Should Contain    ${output}    priority : 53949519
    Should Contain X Times    ${output}    device :    2
    Should Contain    ${output}    device : test
    Should Contain X Times    ${output}    severity :    2
    Should Contain    ${output}    severity : 641612754
