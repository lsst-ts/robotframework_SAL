...
Start Sender
    ...
    Write    cd ${SALWorkDir}/${subSystem}/cpp/src
    Comment    Start Sender.
    ${input}=    Write    ./sacpp_${subSystem}_${component}_send test 685219463 53149674
    ${output}=    Read Until Prompt
    Log    ${output}
...
Read Logger
    [Tags]    functional
    Switch Connection    Logger
    ${output}=    Read Until    priority : 53149674
    Log    ${output}
    Should Contain X Times    ${output}    === Event powerError received =     1
    Should Contain    ${output}    device : test
    Should Contain    ${output}    severity : 685219463
    Should Contain    ${output}    priority : 53149674
