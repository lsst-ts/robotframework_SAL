*** Settings ***
Documentation    M1M3_status commander/controller tests.
...
Start Commander
    ...
    ${output}=    Read Until Prompt
    Log    ${output}
    Should Contain X Times    ${output}    === [issueCommandC status] writing a command containing :    1
    Should Contain X Times    ${output}    device : status    1
    Should Contain X Times    ${output}    property :     1
    Should Contain X Times    ${output}    action :     1
    Should Contain X Times    ${output}    value :     1
    Should Contain    ${output}    === command status issued =
...
Read Controller
    ...
    Log    ${output}
    Should Contain    ${output}    === [acceptCommandC status] reading a command containing :
    Should Contain X Times    ${output}    seqNum \ \ :    3
    Should Contain X Times    ${output}    error \ \ \ :    2
    Should Contain    ${output}    device : status
    Should Contain    ${output}    device \ \ : status
    ...
    Should Contain    ${output}    value \ \ \ : 
    Should Contain    ${output}    === command status received =
