*** Settings ***
Documentation    Dome_Track commander/controller tests.
...
Start Commander
    ...
    ${output}=    Read Until Prompt
    Log    ${output}
    Should Contain X Times    ${output}    === [issueCommandC Track] writing a command containing :    1
    Should Contain X Times    ${output}    device : track    1
    Should Contain X Times    ${output}    property : mode    1
    Should Contain X Times    ${output}    action :     1
    Should Contain X Times    ${output}    value :     1
    Should Contain X Times    ${output}    mode : test    1
...
Read Controller
    ...
    Log    ${output}
    Should Contain    ${output}    === [acceptCommandC Track] reading a command containing :
    Should Contain X Times    ${output}    seqNum \ \ :    3
    Should Contain X Times    ${output}    error \ \ \ :    2
    Should Contain    ${output}    device : track
    Should Contain    ${output}    device \ \ : track
    ...
    Should Contain    ${output}    value \ \ \ : 
    Should Contain    ${output}    === command Track received =
    Should Contain X Times    ${output}    mode : test    1
