*** Settings ***
Documentation    MTMount_closeMirrorCover commander/controller tests.
...
Start Commander
    ...
    ${output}=    Read Until Prompt
    Log    ${output}
    Should Contain X Times    ${output}    === [issueCommandC closeMirrorCover] writing a command containing :    1
    Should Contain X Times    ${output}    device : mirrorCover    1
    Should Contain X Times    ${output}    property : position    1
    Should Contain X Times    ${output}    action :     1
    Should Contain X Times    ${output}    value :     1
    Should Contain    ${output}    === command closeMirrorCover issued =
...
Read Controller
    ...
    Log    ${output}
    Should Contain    ${output}    === [acceptCommandC closeMirrorCover] reading a command containing :
    Should Contain X Times    ${output}    seqNum \ \ :    3
    Should Contain X Times    ${output}    error \ \ \ :    2
    Should Contain    ${output}    device : mirrorCover
    Should Contain    ${output}    device \ \ : mirrorCover
    ...
    Should Contain    ${output}    value \ \ \ : 
    Should Contain    ${output}    === command closeMirrorCover received =
