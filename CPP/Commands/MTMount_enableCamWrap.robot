*** Settings ***
Documentation    MTMount_enableCamWrap commander/controller tests.
...
Start Commander
    ...
    ${output}=    Read Until Prompt
    Log    ${output}
    Should Contain X Times    ${output}    === [issueCommandC enableCamWrap] writing a command containing :    1
    Should Contain X Times    ${output}    device : wrap    1
    Should Contain X Times    ${output}    property : mode    1
    Should Contain X Times    ${output}    action :     1
    Should Contain X Times    ${output}    value :     1
    Should Contain    ${output}    === command enableCamWrap issued =
...
Read Controller
    ...
    Log    ${output}
    Should Contain    ${output}    === [acceptCommandC enableCamWrap] reading a command containing :
    Should Contain X Times    ${output}    seqNum \ \ :    3
    Should Contain X Times    ${output}    error \ \ \ :    2
    Should Contain    ${output}    device : wrap
    Should Contain    ${output}    device \ \ : wrap
    Should Contain X Times    ${output}    property : mode    2
    Should Contain    ${output}    action : 
    Should Contain    ${output}    action \ \ : 
    Should Contain    ${output}    value : 
    Should Contain    ${output}    value \ \ \ : 
    Should Contain    ${output}    === command enableCamWrap received =
