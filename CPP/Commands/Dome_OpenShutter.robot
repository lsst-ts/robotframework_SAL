*** Settings ***
Documentation    Dome_OpenShutter commander/controller tests.
...
Start Commander
    ...
    ${output}=    Read Until Prompt
    Log    ${output}
    Should Contain X Times    ${output}    === [issueCommandC OpenShutter] writing a command containing :    1
    Should Contain X Times    ${output}    device : shutter    1
    Should Contain X Times    ${output}    property : position    1
    Should Contain X Times    ${output}    action :     1
    Should Contain X Times    ${output}    value :     1
    Should Contain    ${output}    === command OpenShutter issued =
...
Read Controller
    ...
    Log    ${output}
    Should Contain    ${output}    === [acceptCommandC OpenShutter] reading a command containing :
