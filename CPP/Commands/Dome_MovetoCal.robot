*** Settings ***
Documentation    Dome_MovetoCal commander/controller tests.
...
Start Commander
    ...
    ${output}=    Read Until Prompt
    Log    ${output}
    Should Contain X Times    ${output}    === [issueCommandC MovetoCal] writing a command containing :    1
    Should Contain X Times    ${output}    device : target    1
    Should Contain X Times    ${output}    property : position    1
    Should Contain X Times    ${output}    action :     1
    Should Contain X Times    ${output}    value :     1
    Should Contain    ${output}    === command MovetoCal issued =
...
Read Controller
    ...
    Log    ${output}
    Should Contain    ${output}    === [acceptCommandC MovetoCal] reading a command containing :
    ...
    Should Contain    ${output}    value \ \ \ : 
    Should Contain    ${output}    === command MovetoCal received =
