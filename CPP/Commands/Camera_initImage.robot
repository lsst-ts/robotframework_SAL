*** Settings ***
Documentation    Camera_initImage commander/controller tests.
...
Start Commander
    ...
    Write    cd ${SALWorkDir}/${subSystem}/cpp/src
    Comment    Start Commander.
    ${input}=    Write    ./sacpp_${subSystem}_${component}_commander 41.4885    #|tee ${comOut}
    ${output}=    Read Until Prompt
    Log    ${output}
    Should Contain X Times    ${output}    === [issueCommandC initImage] writing a command containing :    1
    Should Contain X Times    ${output}    device : initImage    1
    Should Contain X Times    ${output}    property :     1
    Should Contain X Times    ${output}    action :     1
    Should Contain X Times    ${output}    value :     1
    Should Contain X Times    ${output}    deltaT : 41.4885    1
    Should Contain    ${output}    === command initImage issued =
...
Read Controller
    ...
    Log    ${output}
    Should Contain    ${output}    === [acceptCommandC initImage] reading a command containing :
    Should Contain X Times    ${output}    seqNum \ \ :    3
    Should Contain X Times    ${output}    error \ \ \ :    2
    Should Contain    ${output}    device : initImage
    Should Contain    ${output}    device \ \ : initImage
    ...
    Should Contain    ${output}    value \ \ \ : 
    Should Contain    ${output}    === command initImage received =
    Should Contain X Times    ${output}    deltaT : 41.4885    1
