*** Settings ***
Documentation    DomeAPS_Close commander/controller tests.
Force Tags    cpp
...
Start Commander
    ...
    Should Contain    ${output}    === command Close issued =
    ${CmdComplete}=    Get Line    ${output}    -2
...
Read Controller
    ...
    Log    ${output}
    Should Contain    ${output}    === command Close received =
    Should Contain    ${output}    device : aps
    Should Contain    ${output}    property : position
    Should Contain    ${output}    action : 
    Should Contain    ${output}    value : 
    Should Contain X Times    ${output}    state : 0    1
    Should Contain X Times    ${output}    === [ackCommand_Close] acknowledging a command with :    2
