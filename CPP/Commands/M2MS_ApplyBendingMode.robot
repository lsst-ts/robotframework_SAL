...
Start Commander
    ...
    Write    cd ${SALWorkDir}/${subSystem}/cpp/src
    Comment    Start Commander.
    ${input}=    Write    ./sacpp_${subSystem}_${component}_commander 77 39.29    #|tee ${comOut}
    ${output}=    Read Until Prompt
    Log    ${output}
    Should Contain X Times    ${output}    === [issueCommandC ApplyBendingMode] writing a command containing :    1
    Should Contain X Times    ${output}    device : support    1
    Should Contain X Times    ${output}    property : actuators    1
    Should Contain X Times    ${output}    action :     1
    Should Contain X Times    ${output}    value :     1
    Should Contain X Times    ${output}    bendingModeNbr : 77    1
    Should Contain X Times    ${output}    bendingModeValue : 39.29    1
...
Read Controller
    ...
    Log    ${output}
    Should Contain    ${output}    === [acceptCommandC ApplyBendingMode] reading a command containing :
