...
Start settingVersions Sender
    ...
    ${input}=    Write    python ${subSystem}_Event_settingVersions.py   827108189
...
Start errorCode Sender
    ...
    Write    cd ${SALWorkDir}/${subSystem}/python
    Comment    Start Sender.
    ${input}=    Write    python ${subSystem}_Event_errorCode.py -998404667  GdZwbjXQxbytVPNizFLsJjcTcKgseTqsEuBTnnnicyqTtZVQDczpUzfNximlrpaHsERgRpGmgWIUyNMLSUGRKZcuvQzkSqNdNGBYmXECJonBxUPAMXZMFOXBCCSFYVXqAFGimFMvynS 1757916599
    ${output}=    Read Until Prompt
    Log    ${output}
...
Start summaryState Sender
    ...
    Write    cd ${SALWorkDir}/${subSystem}/python
    Comment    Start Sender.
    ${input}=    Write    python ${subSystem}_Event_summaryState.py -1714741527 -1057159950
    ${output}=    Read Until Prompt
    Log    ${output}
...
Start appliedSettingsMatchStart Sender
    ...
    Write    cd ${SALWorkDir}/${subSystem}/python
    Comment    Start Sender.
    ${input}=    Write    python ${subSystem}_Event_appliedSettingsMatchStart.py 1 -1151678016
    ${output}=    Read Until Prompt
    Log    ${output}
