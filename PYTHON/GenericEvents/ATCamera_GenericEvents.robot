...
Start settingVersions Sender
    ...
    ${input}=    Write    python ${subSystem}_Event_settingVersions.py   -973919777
...
Start errorCode Sender
    ...
    Write    cd ${SALWorkDir}/${subSystem}/python
    Comment    Start Sender.
    ${input}=    Write    python ${subSystem}_Event_errorCode.py 1974068376 gPBYxrVXeXIzEmRnKLrFEhKMYABBBgudDmJHGsozKQlbxGWthcYMpLNinEpElNWljEoDfMTJhPHSgNmyFqXAwbzkTGBMyDjcLpbXWXlxTEaxiXcfdqJuezvgqdMxfpXNkSTXSyMDpnxGYoVYBJpiVLRXLzSbAzTvusCwKasFYAowVvBvgVQewOtAUxliFkjquUIyCbDZIrmmzvpynWPvnzpanoGeqAAIpsnxuH  1162107902
    ${output}=    Read Until Prompt
    Log    ${output}
...
Start summaryState Sender
    ...
    Write    cd ${SALWorkDir}/${subSystem}/python
    Comment    Start Sender.
    ${input}=    Write    python ${subSystem}_Event_summaryState.py -814475763 -2069060794
    ${output}=    Read Until Prompt
    Log    ${output}
...
Start appliedSettingsMatchStart Sender
    ...
    Write    cd ${SALWorkDir}/${subSystem}/python
    Comment    Start Sender.
    ${input}=    Write    python ${subSystem}_Event_appliedSettingsMatchStart.py 0 1940634877
    ${output}=    Read Until Prompt
    Log    ${output}
