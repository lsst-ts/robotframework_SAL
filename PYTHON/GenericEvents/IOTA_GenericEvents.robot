...
Start settingVersions Sender
    ...
    ${input}=    Write    python ${subSystem}_Event_settingVersions.py   -357964348
...
Start errorCode Sender
    ...
    Write    cd ${SALWorkDir}/${subSystem}/python
    Comment    Start Sender.
    ${input}=    Write    python ${subSystem}_Event_errorCode.py 186419954  ZcsNyWeNHnMKHuHxrfxdboLgEILheXhadEwTobekzuhgVAQrAUATYIKhkNdDdxhQEWOCJboZJQGYxuICmTHFQHEKixowJwLJWPSHaObqmOvCPUoDRwzCrJrYgdjGWfc 560527591
    ${output}=    Read Until Prompt
    Log    ${output}
...
Start summaryState Sender
    ...
    Write    cd ${SALWorkDir}/${subSystem}/python
    Comment    Start Sender.
    ${input}=    Write    python ${subSystem}_Event_summaryState.py 1744914634 -2033170302
    ${output}=    Read Until Prompt
    Log    ${output}
...
Start appliedSettingsMatchStart Sender
    ...
    Write    cd ${SALWorkDir}/${subSystem}/python
    Comment    Start Sender.
    ${input}=    Write    python ${subSystem}_Event_appliedSettingsMatchStart.py 1 202043014
    ${output}=    Read Until Prompt
    Log    ${output}
