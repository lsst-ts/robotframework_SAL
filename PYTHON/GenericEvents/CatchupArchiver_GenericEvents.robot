...
Start settingVersions Sender
    ...
    ${input}=    Write    python ${subSystem}_Event_settingVersions.py   319584450
...
Start errorCode Sender
    ...
    Write    cd ${SALWorkDir}/${subSystem}/python
    Comment    Start Sender.
    ${input}=    Write    python ${subSystem}_Event_errorCode.py 961096591 uvfVHeeLkAWyigrWnJoSrxCBmAPSvzFvWyiOIRmdqNBGtWFYlWrpeemJBaEULRFzzRPlaNYXmrewOTmgveHvbuMHkAzJZUHdUtberhMLbVUWTbrWxQGFsVILyLfFSoHYwyQNxoxuvhQpuEZXAOmqdNQfUjsKaFPsZkWwzExQHWSTUqYu yiepydIJWPTqVPCavmVpnguRuzngpMhKzfVupdbQvOxBHuucBoxSozkTtjvPeGhDcnuOyoJcAPQRJRVEbDXrsJZKNhCiPFzmoZxthidQaCVOhDtSYwVlenmIpsIQRprNkHMfeaZSpwzRjHRAVHqcNyVtfmwJDeFcJkDkWbrmeTwjaGSLXeUSqmLjwfMXItXP -158055703
    ${output}=    Read Until Prompt
    Log    ${output}
...
Start summaryState Sender
    ...
    Write    cd ${SALWorkDir}/${subSystem}/python
    Comment    Start Sender.
    ${input}=    Write    python ${subSystem}_Event_summaryState.py -1438775612 493692267
    ${output}=    Read Until Prompt
    Log    ${output}
...
Start appliedSettingsMatchStart Sender
    ...
    Write    cd ${SALWorkDir}/${subSystem}/python
    Comment    Start Sender.
    ${input}=    Write    python ${subSystem}_Event_appliedSettingsMatchStart.py 1 414335965
    ${output}=    Read Until Prompt
    Log    ${output}
