...
Start settingVersions Sender
    ...
    ${input}=    Write    python ${subSystem}_Event_settingVersions.py   768695111
...
Start errorCode Sender
    ...
    Write    cd ${SALWorkDir}/${subSystem}/python
    Comment    Start Sender.
    ${input}=    Write    python ${subSystem}_Event_errorCode.py -2126407677  JnrYWOXXoWzjuYwZdmfHwghMRwoxOUJcJPUmMpTFxBmwIfsERxGicnvJwgSBlsahHVxGLdemPtujbUEqMzsJJOtuSUeyPdbSPVRTQIBjNJaRfULoPMSxtcTHtyWOZzxUdDXGoVpKtzNdpORIN -54797150
    ${output}=    Read Until Prompt
    Log    ${output}
...
Start summaryState Sender
    ...
    Write    cd ${SALWorkDir}/${subSystem}/python
    Comment    Start Sender.
    ${input}=    Write    python ${subSystem}_Event_summaryState.py -312208723 1430444451
    ${output}=    Read Until Prompt
    Log    ${output}
...
Start appliedSettingsMatchStart Sender
    ...
    Write    cd ${SALWorkDir}/${subSystem}/python
    Comment    Start Sender.
    ${input}=    Write    python ${subSystem}_Event_appliedSettingsMatchStart.py 1 -1987291063
    ${output}=    Read Until Prompt
    Log    ${output}
