...
Start settingVersions Sender
    ...
    ${input}=    Write    python ${subSystem}_Event_settingVersions.py   -1999294998
...
Start errorCode Sender
    ...
    Write    cd ${SALWorkDir}/${subSystem}/python
    Comment    Start Sender.
    ${input}=    Write    python ${subSystem}_Event_errorCode.py 381475933 lkYDEOz PhymEWzLnduobVtmULtpIpWnxjQdKJFvowpyYCjQmADzVfKyDDFOAuzTJWyWUDKkIFMgnVVLrpvTkoljNsJniemvWlsNUUTZHiLDXlvDTRfjlFmIqYsJuqGibsGMYAOifwYVjNomaIDQkgOrPkxEAWYtBxsyUqtkmRjqkjMXeikiMlebedPQYnfzEgXoaZnyvcqaFDofgeBywMOcSHPjAcjkphiTNpHwfXRhlBIBuCvLaXzxukbivGYovmBUTlD 492043236
    ${output}=    Read Until Prompt
    Log    ${output}
...
Start summaryState Sender
    ...
    Write    cd ${SALWorkDir}/${subSystem}/python
    Comment    Start Sender.
    ${input}=    Write    python ${subSystem}_Event_summaryState.py 1457730656 2010822784
    ${output}=    Read Until Prompt
    Log    ${output}
...
Start appliedSettingsMatchStart Sender
    ...
    Write    cd ${SALWorkDir}/${subSystem}/python
    Comment    Start Sender.
    ${input}=    Write    python ${subSystem}_Event_appliedSettingsMatchStart.py 1 -402257504
    ${output}=    Read Until Prompt
    Log    ${output}
