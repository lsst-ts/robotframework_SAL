...
Start settingVersions Sender
    ...
    ${input}=    Write    python ${subSystem}_Event_settingVersions.py   610315192
...
Start errorCode Sender
    ...
    Write    cd ${SALWorkDir}/${subSystem}/python
    Comment    Start Sender.
    ${input}=    Write    python ${subSystem}_Event_errorCode.py -440594866 jnTlWAUVXHwHWWKCqouUONaELRtufMBOgmhtDAXBTPlAyTHzcGshadCBorCSYxOBrpkAUxQZEaafGlEYDFfGHwJdMcyxaDngYfSTM  1320643193
    ${output}=    Read Until Prompt
    Log    ${output}
...
Start summaryState Sender
    ...
    Write    cd ${SALWorkDir}/${subSystem}/python
    Comment    Start Sender.
    ${input}=    Write    python ${subSystem}_Event_summaryState.py 1841738397 716295235
    ${output}=    Read Until Prompt
    Log    ${output}
...
Start appliedSettingsMatchStart Sender
    ...
    Write    cd ${SALWorkDir}/${subSystem}/python
    Comment    Start Sender.
    ${input}=    Write    python ${subSystem}_Event_appliedSettingsMatchStart.py 1 24110840
    ${output}=    Read Until Prompt
    Log    ${output}
