...
Start settingVersions Sender
    ...
    ${input}=    Write    python ${subSystem}_Event_settingVersions.py   -785603701
...
Start errorCode Sender
    ...
    Write    cd ${SALWorkDir}/${subSystem}/python
    Comment    Start Sender.
    ${input}=    Write    python ${subSystem}_Event_errorCode.py -1004523339 FPePNVekuThSDTTpVUPOGeBXMzsNmWhbkHgyVanvfEloFcHHWjqyUWhcWbjqefrPkoumGSQauWQSxMMaborRDylMIMczPQNyMOecxDlAIqhuWDsfpszWZbtRxAJXqzTseAsBSXrlPVAfrjCLPpfjQKKLgsrKHshospMdBWZTaMRLUUnhOgHUUqEAQhXYGFcWdyqlCGNHBYvzW HzyNZokoypxPnxfXrbMWkXYMXjaTnqcicyzuZYWweSTbgxtmuRfHOpwbmXEvLgsXdCOLfGoxerXLVCelgHeATMzjsxUVNtwVNmreQvSasDSnWPQYbvyWmgJHnytSyBeBdRHujURkQNeCOnTBVBlaVnkuhLdOMqqYpvRcQhnXsivPNTGHyFiCCELlHkWHVtxhihKsXLSJFWtpqwQiTevMlxfhkoSgb 1067466466
    ${output}=    Read Until Prompt
    Log    ${output}
...
Start summaryState Sender
    ...
    Write    cd ${SALWorkDir}/${subSystem}/python
    Comment    Start Sender.
    ${input}=    Write    python ${subSystem}_Event_summaryState.py -1554329397 2111253246
    ${output}=    Read Until Prompt
    Log    ${output}
...
Start appliedSettingsMatchStart Sender
    ...
    Write    cd ${SALWorkDir}/${subSystem}/python
    Comment    Start Sender.
    ${input}=    Write    python ${subSystem}_Event_appliedSettingsMatchStart.py 1 -1725047573
    ${output}=    Read Until Prompt
    Log    ${output}
