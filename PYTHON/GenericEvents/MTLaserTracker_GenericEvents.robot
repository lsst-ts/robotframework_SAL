...
Start settingVersions Sender
    ...
    ${input}=    Write    python ${subSystem}_Event_settingVersions.py   -1096140219
...
Start errorCode Sender
    ...
    Write    cd ${SALWorkDir}/${subSystem}/python
    Comment    Start Sender.
    ${input}=    Write    python ${subSystem}_Event_errorCode.py -2112420315  QCMSoKTHZSexngQflJFaDHtHmPIQxHIlJZXJSdAYgUBIINisMVhrffDXXPsnPgmokZuMGSQZCAWyZfxzAjcKbjNkyLsKpIParlyFnCdjdXBYZRNmURbbyPRsScpbUQcnbRpMrAVECPHWtFgTxzZDueSTRCRAd 1545364104
    ${output}=    Read Until Prompt
    Log    ${output}
...
Start summaryState Sender
    ...
    Write    cd ${SALWorkDir}/${subSystem}/python
    Comment    Start Sender.
    ${input}=    Write    python ${subSystem}_Event_summaryState.py -921214063 2045732562
    ${output}=    Read Until Prompt
    Log    ${output}
...
Start appliedSettingsMatchStart Sender
    ...
    Write    cd ${SALWorkDir}/${subSystem}/python
    Comment    Start Sender.
    ${input}=    Write    python ${subSystem}_Event_appliedSettingsMatchStart.py 0 61250109
    ${output}=    Read Until Prompt
    Log    ${output}
