...
Start settingVersions Sender
    ...
    ${input}=    Write    python ${subSystem}_Event_settingVersions.py   -1594159461
...
Start errorCode Sender
    ...
    Write    cd ${SALWorkDir}/${subSystem}/python
    Comment    Start Sender.
    ${input}=    Write    python ${subSystem}_Event_errorCode.py 805208705 ILkgplYRXXbVpFxxPFUnHpQqBhNLJxJJOOfQWaIvimWPQaKgUCAwhCPIWDOIUgOvgAFGBOULAJCNFtGQTIEFLRNylZvKhMpgtJhcJxrZQPSoszSfrsAdpagFTpkARMeHpFopdRRNzXOZlPFNoROsamAfzYxqOheXYPKqY  2099259685
    ${output}=    Read Until Prompt
    Log    ${output}
...
Start summaryState Sender
    ...
    Write    cd ${SALWorkDir}/${subSystem}/python
    Comment    Start Sender.
    ${input}=    Write    python ${subSystem}_Event_summaryState.py -1974876592 619826247
    ${output}=    Read Until Prompt
    Log    ${output}
...
Start appliedSettingsMatchStart Sender
    ...
    Write    cd ${SALWorkDir}/${subSystem}/python
    Comment    Start Sender.
    ${input}=    Write    python ${subSystem}_Event_appliedSettingsMatchStart.py 1 1019884783
    ${output}=    Read Until Prompt
    Log    ${output}
