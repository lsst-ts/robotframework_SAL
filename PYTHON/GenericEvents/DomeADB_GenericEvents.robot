...
Start settingVersions Sender
    ...
    ${input}=    Write    python ${subSystem}_Event_settingVersions.py   -1199685735
...
Start errorCode Sender
    ...
    Write    cd ${SALWorkDir}/${subSystem}/python
    Comment    Start Sender.
    ${input}=    Write    python ${subSystem}_Event_errorCode.py 188700779  cPOHGVXGuVIdChzrIrcbjeKdIcmKgJllNYvcTVffNjvzhvIDpnOFRgZBjRQVykPrgFAbhcFaUAhiYiQBELivGZFiNfYUUfomNksJyFTROiDmLCGQvDbpbcGOfswnOxkXgQAMuskoSpNLpVJOVtWoCPHIGnJMmOxkEGcQPtFT -334342973
    ${output}=    Read Until Prompt
    Log    ${output}
...
Start summaryState Sender
    ...
    Write    cd ${SALWorkDir}/${subSystem}/python
    Comment    Start Sender.
    ${input}=    Write    python ${subSystem}_Event_summaryState.py -1294012693 1665379474
    ${output}=    Read Until Prompt
    Log    ${output}
...
Start appliedSettingsMatchStart Sender
    ...
    Write    cd ${SALWorkDir}/${subSystem}/python
    Comment    Start Sender.
    ${input}=    Write    python ${subSystem}_Event_appliedSettingsMatchStart.py 0 704403636
    ${output}=    Read Until Prompt
    Log    ${output}
