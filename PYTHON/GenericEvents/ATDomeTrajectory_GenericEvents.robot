...
Start settingVersions Sender
    ...
    ${input}=    Write    python ${subSystem}_Event_settingVersions.py   -387719881
...
Start errorCode Sender
    ...
    Write    cd ${SALWorkDir}/${subSystem}/python
    Comment    Start Sender.
    ${input}=    Write    python ${subSystem}_Event_errorCode.py 1960235903 wWMgiqVuOEwyNhROcYStDFGuwoYlJrTWaSjeyAmgxxywXrhRMuGJNkcKJmAOTzyTSBoqtUQVCLZTTWVCiHVGvVBBhZUJMvVlzKcVmNCGwyXBMADgJGAVqdXzUHImNmhuItEfhkIHMRnhYjtUrFm  -1059298413
    ${output}=    Read Until Prompt
    Log    ${output}
...
Start summaryState Sender
    ...
    Write    cd ${SALWorkDir}/${subSystem}/python
    Comment    Start Sender.
    ${input}=    Write    python ${subSystem}_Event_summaryState.py 356925650 773867242
    ${output}=    Read Until Prompt
    Log    ${output}
...
Start appliedSettingsMatchStart Sender
    ...
    Write    cd ${SALWorkDir}/${subSystem}/python
    Comment    Start Sender.
    ${input}=    Write    python ${subSystem}_Event_appliedSettingsMatchStart.py 1 -432529052
    ${output}=    Read Until Prompt
    Log    ${output}
