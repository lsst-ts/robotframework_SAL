...
Start settingVersions Sender
    ...
    ${input}=    Write    python ${subSystem}_Event_settingVersions.py   272146407
...
Start errorCode Sender
    ...
    Write    cd ${SALWorkDir}/${subSystem}/python
    Comment    Start Sender.
    ${input}=    Write    python ${subSystem}_Event_errorCode.py -2067355436  fmqbKizOpAbfVSFhLXvELVTuQxqgxgAnbhyXiQmebWrvUOjbvarIkHzIWQBLcWzIPYlpSlFslZVaGwEKJGigOXrDRqvjwkixCngLaIefzWaqLFBjmgQbxdvXmAhJcBFJtisYLoZRDqqWfCmzVzaXEwJMRIscgBYaNBWtSnyXXtBJLfwOUlLijkCmYUyNbWsouHPTSmhMTGBaRgBAeJndnBdyJEiQYne 651967278
    ${output}=    Read Until Prompt
    Log    ${output}
...
Start summaryState Sender
    ...
    Write    cd ${SALWorkDir}/${subSystem}/python
    Comment    Start Sender.
    ${input}=    Write    python ${subSystem}_Event_summaryState.py 569071923 -2036312782
    ${output}=    Read Until Prompt
    Log    ${output}
...
Start appliedSettingsMatchStart Sender
    ...
    Write    cd ${SALWorkDir}/${subSystem}/python
    Comment    Start Sender.
    ${input}=    Write    python ${subSystem}_Event_appliedSettingsMatchStart.py 0 1826977983
    ${output}=    Read Until Prompt
    Log    ${output}
