...
Start settingVersions Sender
    ...
    ${input}=    Write    python ${subSystem}_Event_settingVersions.py   -701679677
...
Start errorCode Sender
    ...
    Write    cd ${SALWorkDir}/${subSystem}/python
    Comment    Start Sender.
    ${input}=    Write    python ${subSystem}_Event_errorCode.py 39956928 sEdvdpYtYZiMFGRgyTDdMwCZfkDgKlFCUSYDtDERVEmxjtmHIcPFAOMfUy LuXpnDeTkHdQVqOePVNXoPYQQUJKUVeeqspQLcSHXNdcGzVeReeYpaTKsBqVakskeWIFrGYXg 2078972608
    ${output}=    Read Until Prompt
    Log    ${output}
...
Start summaryState Sender
    ...
    Write    cd ${SALWorkDir}/${subSystem}/python
    Comment    Start Sender.
    ${input}=    Write    python ${subSystem}_Event_summaryState.py -2047741134 928103648
    ${output}=    Read Until Prompt
    Log    ${output}
...
Start appliedSettingsMatchStart Sender
    ...
    Write    cd ${SALWorkDir}/${subSystem}/python
    Comment    Start Sender.
    ${input}=    Write    python ${subSystem}_Event_appliedSettingsMatchStart.py 0 -455473989
    ${output}=    Read Until Prompt
    Log    ${output}
