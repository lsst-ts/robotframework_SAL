...
Start settingVersions Sender
    ...
    ${input}=    Write    python ${subSystem}_Event_settingVersions.py   -1452852045
...
Start errorCode Sender
    ...
    Write    cd ${SALWorkDir}/${subSystem}/python
    Comment    Start Sender.
    ${input}=    Write    python ${subSystem}_Event_errorCode.py -21131677 jHLzfrVlNhjGpNmmLPinlRZRAGLXGIyYrrqrCOHBgyYbLYkkvwQcMMIHbLcPpqtjekYuYKJySpYwxeVopjxixWBPyYdFqntvhBqkERt uWLBjVSqNDuFGgazoGmOAbZpuonTtFGfSqJSgSoRdApqHYyXrygXzIByXZLZKYOTRjgUkRikddjfvnNbCgpPiauCNpEVPZVgPwYjobKxdjNfeOtRmPszyW 252866224
    ${output}=    Read Until Prompt
    Log    ${output}
...
Start summaryState Sender
    ...
    Write    cd ${SALWorkDir}/${subSystem}/python
    Comment    Start Sender.
    ${input}=    Write    python ${subSystem}_Event_summaryState.py 1294543762 1060444928
    ${output}=    Read Until Prompt
    Log    ${output}
...
Start appliedSettingsMatchStart Sender
    ...
    Write    cd ${SALWorkDir}/${subSystem}/python
    Comment    Start Sender.
    ${input}=    Write    python ${subSystem}_Event_appliedSettingsMatchStart.py 1 -334868440
    ${output}=    Read Until Prompt
    Log    ${output}
