*** Settings ***
Documentation    M1M3_AppliedVelocityForces sender/logger tests.
...
Start Sender
    ...
    Log    ${output}
    Should Contain X Times    ${output}    === [putSample logevent_AppliedVelocityForces] writing a message containing :    1
...
Read Logger
    ...
    ${output}=    Read Until Prompt
    Log    ${output}
    Should Contain    ${output}    Running ${subSystem}EventLogger_AppliedVelocityForcesTest
