*** Settings ***
Documentation    M1M3_RejectedAzimuthForces sender/logger tests.
...
Start Sender
    ...
    Log    ${output}
    Should Contain X Times    ${output}    === [putSample logevent_RejectedAzimuthForces] writing a message containing :    1
...
Read Logger
    ...
    ${output}=    Read Until Prompt
    Log    ${output}
    Should Contain    ${output}    Running ${subSystem}EventLogger_RejectedAzimuthForcesTest
