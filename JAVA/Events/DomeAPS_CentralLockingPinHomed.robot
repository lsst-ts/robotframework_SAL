...
Start Sender
    ...
    Log    ${output}
    Should Contain X Times    ${output}    === [putSample logevent_CentralLockingPinHomed] writing a message containing :    1
...
Read Logger
    ...
    ${output}=    Read Until Prompt
    Log    ${output}
    Should Contain    ${output}    Running ${subSystem}EventLogger_CentralLockingPinHomedTest
