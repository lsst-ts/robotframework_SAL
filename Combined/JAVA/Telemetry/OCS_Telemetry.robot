*** Settings ***
Documentation    OCS_Telemetry communications tests.
Force Tags    java    
...
Read Subscriber
    ...
    Should Contain    ${output.stdout}    ===== OCS all subscribers ready =====
    @{full_list}=    Split To Lines    ${output.stdout}    start=34
    ${sequencerHeartbeat_start}=    Get Index From List    ${full_list}    === OCS_sequencerHeartbeat start of topic ===
    ${sequencerHeartbeat_end}=    Get Index From List    ${full_list}    === OCS_sequencerHeartbeat end of topic ===
    ...
    Should Contain    ${sequencerHeartbeat_list}    === OCS_sequencerHeartbeat start of topic ===
    Should Contain    ${sequencerHeartbeat_list}    === OCS_sequencerHeartbeat end of topic ===
