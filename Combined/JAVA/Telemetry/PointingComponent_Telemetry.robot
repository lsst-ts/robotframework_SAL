*** Settings ***
Documentation    PointingComponent_Telemetry communications tests.
Force Tags    java    
...
Read Subscriber
    ...
    Should Contain    ${output.stdout}    ===== PointingComponent all subscribers ready =====
    @{full_list}=    Split To Lines    ${output.stdout}    start=34
    ${toBeDeleted_start}=    Get Index From List    ${full_list}    === PointingComponent_toBeDeleted start of topic ===
    ${toBeDeleted_end}=    Get Index From List    ${full_list}    === PointingComponent_toBeDeleted end of topic ===
    ${toBeDeleted_list}=    Get Slice From List    ${full_list}    start=${toBeDeleted_start}    end=${toBeDeleted_end + 1}
    Log Many    ${toBeDeleted_list}
    Should Contain    ${toBeDeleted_list}    === PointingComponent_toBeDeleted start of topic ===
    Should Contain    ${toBeDeleted_list}    === PointingComponent_toBeDeleted end of topic ===
