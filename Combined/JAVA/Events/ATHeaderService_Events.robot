...
Start Sender
    ...
    FOR    ${i}    IN RANGE    30
        Exit For Loop If     'ATHeaderService all loggers ready' in $loggerOutput
        ${loggerOutput}=    Get File    ${EXECDIR}${/}stdoutLogger.txt
        Sleep    3s
    END
    ...
    ${settingVersions_start}=    Get Index From List    ${full_list}    === ATHeaderService_settingVersions start of topic ===
    ${settingVersions_end}=    Get Index From List    ${full_list}    === ATHeaderService_settingVersions end of topic ===
    ...
    Should Contain    ${settingVersions_list}    === ATHeaderService_settingVersions start of topic ===
    Should Contain    ${settingVersions_list}    === ATHeaderService_settingVersions end of topic ===
    Should Contain    ${settingVersions_list}    === [putSample logevent_settingVersions] writing a message containing :
    ${errorCode_start}=    Get Index From List    ${full_list}    === ATHeaderService_errorCode start of topic ===
    ${errorCode_end}=    Get Index From List    ${full_list}    === ATHeaderService_errorCode end of topic ===
    ${errorCode_list}=    Get Slice From List    ${full_list}    start=${errorCode_start}    end=${errorCode_end + 1}
    Log Many    ${errorCode_list}
    Should Contain    ${errorCode_list}    === ATHeaderService_errorCode start of topic ===
    Should Contain    ${errorCode_list}    === ATHeaderService_errorCode end of topic ===
    Should Contain    ${errorCode_list}    === [putSample logevent_errorCode] writing a message containing :
    ${summaryState_start}=    Get Index From List    ${full_list}    === ATHeaderService_summaryState start of topic ===
    ${summaryState_end}=    Get Index From List    ${full_list}    === ATHeaderService_summaryState end of topic ===
    ...
    Should Contain    ${summaryState_list}    === ATHeaderService_summaryState start of topic ===
    Should Contain    ${summaryState_list}    === ATHeaderService_summaryState end of topic ===
    Should Contain    ${summaryState_list}    === [putSample logevent_summaryState] writing a message containing :
    ${appliedSettingsMatchStart_start}=    Get Index From List    ${full_list}    === ATHeaderService_appliedSettingsMatchStart start of topic ===
    ${appliedSettingsMatchStart_end}=    Get Index From List    ${full_list}    === ATHeaderService_appliedSettingsMatchStart end of topic ===
    ...
    Should Contain    ${appliedSettingsMatchStart_list}    === ATHeaderService_appliedSettingsMatchStart start of topic ===
    Should Contain    ${appliedSettingsMatchStart_list}    === ATHeaderService_appliedSettingsMatchStart end of topic ===
    ...
    Should Contain    ${logLevel_list}    === ATHeaderService_logLevel start of topic ===
    Should Contain    ${logLevel_list}    === ATHeaderService_logLevel end of topic ===
    Should Contain    ${logLevel_list}    === [putSample logevent_logLevel] writing a message containing :
    ${logMessage_start}=    Get Index From List    ${full_list}    === ATHeaderService_logMessage start of topic ===
    ${logMessage_end}=    Get Index From List    ${full_list}    === ATHeaderService_logMessage end of topic ===
    ${logMessage_list}=    Get Slice From List    ${full_list}    start=${logMessage_start}    end=${logMessage_end + 1}
    Log Many    ${logMessage_list}
    Should Contain    ${logMessage_list}    === ATHeaderService_logMessage start of topic ===
    Should Contain    ${logMessage_list}    === ATHeaderService_logMessage end of topic ===
    Should Contain    ${logMessage_list}    === [putSample logevent_logMessage] writing a message containing :
    ${settingsApplied_start}=    Get Index From List    ${full_list}    === ATHeaderService_settingsApplied start of topic ===
    ${settingsApplied_end}=    Get Index From List    ${full_list}    === ATHeaderService_settingsApplied end of topic ===
    ...
    Should Contain    ${settingsApplied_list}    === ATHeaderService_settingsApplied start of topic ===
    Should Contain    ${settingsApplied_list}    === ATHeaderService_settingsApplied end of topic ===
    Should Contain    ${settingsApplied_list}    === [putSample logevent_settingsApplied] writing a message containing :
    ${simulationMode_start}=    Get Index From List    ${full_list}    === ATHeaderService_simulationMode start of topic ===
    ${simulationMode_end}=    Get Index From List    ${full_list}    === ATHeaderService_simulationMode end of topic ===
    ...
    Should Contain    ${simulationMode_list}    === ATHeaderService_simulationMode start of topic ===
    Should Contain    ${simulationMode_list}    === ATHeaderService_simulationMode end of topic ===
    Should Contain    ${simulationMode_list}    === [putSample logevent_simulationMode] writing a message containing :
    ${softwareVersions_start}=    Get Index From List    ${full_list}    === ATHeaderService_softwareVersions start of topic ===
    ${softwareVersions_end}=    Get Index From List    ${full_list}    === ATHeaderService_softwareVersions end of topic ===
    ...
    Should Contain    ${softwareVersions_list}    === ATHeaderService_softwareVersions start of topic ===
    Should Contain    ${softwareVersions_list}    === ATHeaderService_softwareVersions end of topic ===
    ...
    ${authList_start}=    Get Index From List    ${full_list}    === ATHeaderService_authList start of topic ===
    ${authList_end}=    Get Index From List    ${full_list}    === ATHeaderService_authList end of topic ===
    ${authList_list}=    Get Slice From List    ${full_list}    start=${authList_start}    end=${authList_end + 1}
    Log Many    ${authList_list}
    Should Contain    ${authList_list}    === ATHeaderService_authList start of topic ===
    Should Contain    ${authList_list}    === ATHeaderService_authList end of topic ===
...
Read Subscriber
    ...
    Log Many    ${output.stdout}    ${output.stderr}
    Should Contain    ${output.stdout}    ===== ATHeaderService all loggers ready =====
    @{full_list}=    Split To Lines    ${output.stdout}    start=29
    ${largeFileObjectAvailable_start}=    Get Index From List    ${full_list}    === ATHeaderService_largeFileObjectAvailable start of topic ===
    ${largeFileObjectAvailable_end}=    Get Index From List    ${full_list}    === ATHeaderService_largeFileObjectAvailable end of topic ===
    ${largeFileObjectAvailable_list}=    Get Slice From List    ${full_list}    start=${largeFileObjectAvailable_start}    end=${largeFileObjectAvailable_end + 1}
