*** Settings ***
Documentation    MTVMS_Commands communications tests.
...
Start Commander
    ...
    Log Many    @{full_list}
    Comment    ======= Verify ${subSystem}_changeSamplePeriod test messages =======
    Should Contain    ${output.stdout}    ==== ${subSystem} all commanders ready ====
    ${changeSamplePeriod_start}=    Get Index From List    ${full_list}    === MTVMS_changeSamplePeriod start of topic ===
    ${line}=    Get Lines Matching Pattern    ${output.stdout}    === waitForCompletion_changeSamplePeriod command*
    ${changeSamplePeriod_end}=    Get Index From List    ${full_list}    ${line}
    ${changeSamplePeriod_list}=    Get Slice From List    ${full_list}    start=${changeSamplePeriod_start}    end=${changeSamplePeriod_end+3}
    Log    ${changeSamplePeriod_list}
    Should Contain X Times    ${changeSamplePeriod_list}    === ${subSystem}_changeSamplePeriod start of topic ===    1
    Should Contain X Times    ${changeSamplePeriod_list}    ${SPACE}${SPACE}${SPACE}${SPACE}period : 1    1
    Should Contain    ${changeSamplePeriod_list}    === issueCommand_changeSamplePeriod writing a command containing :
    Should Contain    ${line}    completed ok
    Should Contain    ${changeSamplePeriod_list}[-2]    Command roundtrip was
    Should Be Equal    ${changeSamplePeriod_list}[-1]    303
    Should Contain    ${changeSamplePeriod_list}    === ${subSystem}_changeSamplePeriod end of topic ===
    Comment    ======= Verify ${subSystem}_disable test messages =======
    Should Contain    ${output.stdout}    ==== ${subSystem} all commanders ready ====
    ${disable_start}=    Get Index From List    ${full_list}    === MTVMS_disable start of topic ===
    ...
    Should Contain    ${output.stdout}    ==== ${subSystem} all commanders ready ====
    ${enable_start}=    Get Index From List    ${full_list}    === MTVMS_enable start of topic ===
    ...
    Should Contain    ${output.stdout}    ==== ${subSystem} all commanders ready ====
    ${exitControl_start}=    Get Index From List    ${full_list}    === MTVMS_exitControl start of topic ===
    ...
    Should Contain    ${output.stdout}    ==== ${subSystem} all commanders ready ====
    ${setLogLevel_start}=    Get Index From List    ${full_list}    === MTVMS_setLogLevel start of topic ===
    ...
    Should Contain    ${output.stdout}    ==== ${subSystem} all commanders ready ====
    ${standby_start}=    Get Index From List    ${full_list}    === MTVMS_standby start of topic ===
    ...
    Should Contain    ${output.stdout}    ==== ${subSystem} all commanders ready ====
    ${start_start}=    Get Index From List    ${full_list}    === MTVMS_start start of topic ===
...
Read Controller
    ...
    ${changeSamplePeriod_start}=    Get Index From List    ${full_list}    === MTVMS_changeSamplePeriod start of topic ===
    ${changeSamplePeriod_end}=    Get Index From List    ${full_list}    === MTVMS_changeSamplePeriod end of topic ===
    ${changeSamplePeriod_list}=    Get Slice From List    ${full_list}    start=${changeSamplePeriod_start}    end=${changeSamplePeriod_end+1}
    Log    ${changeSamplePeriod_list}
    Should Contain X Times    ${changeSamplePeriod_list}    === ${subSystem}_changeSamplePeriod start of topic ===    1
    Should Contain X Times    ${changeSamplePeriod_list}    ${SPACE}${SPACE}${SPACE}${SPACE}period : 1    1
    Should Contain X Times    ${changeSamplePeriod_list}    === ackCommand_changeSamplePeriod acknowledging a command with :    2
    Should Contain X Times    ${changeSamplePeriod_list}    ${SPACE}${SPACE}${SPACE}${SPACE}ack${SPACE}${SPACE}${SPACE}${SPACE}${SPACE}${SPACE}: 301    1
    Should Contain X Times    ${changeSamplePeriod_list}    ${SPACE}${SPACE}${SPACE}${SPACE}ack${SPACE}${SPACE}${SPACE}${SPACE}${SPACE}${SPACE}: 303    1
    Should Contain X Times    ${changeSamplePeriod_list}    ${SPACE}${SPACE}${SPACE}${SPACE}result${SPACE}${SPACE}${SPACE}: Ack : OK    1
    Should Contain X Times    ${changeSamplePeriod_list}    ${SPACE}${SPACE}${SPACE}${SPACE}result${SPACE}${SPACE}${SPACE}: Done : OK    1
    Should Contain X Times    ${changeSamplePeriod_list}    ${SPACE}${SPACE}${SPACE}${SPACE}error${SPACE}${SPACE}${SPACE}${SPACE}: 0    2
    Should Contain X Times    ${changeSamplePeriod_list}    ${SPACE}${SPACE}${SPACE}${SPACE}timeout${SPACE}${SPACE}: 0    2
    Should Contain X Times    ${changeSamplePeriod_list}    === ${subSystem}_changeSamplePeriod end of topic ===    1
    ${disable_start}=    Get Index From List    ${full_list}    === MTVMS_disable start of topic ===
    ${disable_end}=    Get Index From List    ${full_list}    === MTVMS_disable end of topic ===
    ...
    ${enable_start}=    Get Index From List    ${full_list}    === MTVMS_enable start of topic ===
    ${enable_end}=    Get Index From List    ${full_list}    === MTVMS_enable end of topic ===
    ...
    ${exitControl_start}=    Get Index From List    ${full_list}    === MTVMS_exitControl start of topic ===
    ${exitControl_end}=    Get Index From List    ${full_list}    === MTVMS_exitControl end of topic ===
    ...
    ${setLogLevel_start}=    Get Index From List    ${full_list}    === MTVMS_setLogLevel start of topic ===
    ${setLogLevel_end}=    Get Index From List    ${full_list}    === MTVMS_setLogLevel end of topic ===
    ...
    ${standby_start}=    Get Index From List    ${full_list}    === MTVMS_standby start of topic ===
    ${standby_end}=    Get Index From List    ${full_list}    === MTVMS_standby end of topic ===
    ...
    ${start_start}=    Get Index From List    ${full_list}    === MTVMS_start start of topic ===
    ${start_end}=    Get Index From List    ${full_list}    === MTVMS_start end of topic ===
