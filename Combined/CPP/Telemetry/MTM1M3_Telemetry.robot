...
Start Publisher
    ...
    @{words}=    Split String    ${line}
    ${revcode}=    Set Variable    @{words}[2]
    Should Contain    ${output.stdout}    === MTM1M3_powerData start of topic ===
    Should Contain X Times    ${output.stdout}    [putSample] ${subSystem}::powerData_${revcode} writing a message containing :    10
    Should Contain X Times    ${output.stdout}    revCode \ : ${revcode}    10
    Should Contain    ${output.stdout}    === MTM1M3_powerData end of topic ===
...
Read Subscriber
    ...
    ${powerData_list}=    Get Slice From List    ${full_list}    start=${powerData_start}    end=${powerData_end}
