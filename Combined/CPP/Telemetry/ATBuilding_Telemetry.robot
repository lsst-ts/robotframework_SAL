*** Settings ***
Documentation    ATBuilding Telemetry communications tests.
...
Start Subscriber
    ...
    Should Contain    ${output}    ===== ATBuilding subscribers ready =====
...
Start Publisher
    ...
    Should Contain    ${output.stdout}    === ATBuilding_timestamp start of topic ===
    Should Contain X Times    ${output.stdout}    [putSample] ${subSystem}::timestamp_${revcode} writing a message containing :    10
    Should Contain X Times    ${output.stdout}    revCode \ : ${revcode}    10
    Should Contain    ${output.stdout}    === ATBuilding_timestamp end of topic ===
    ...
    @{words}=    Split String    ${line}
    ${revcode}=    Set Variable    @{words}[2]
    Should Contain    ${output.stdout}    === ATBuilding_loopTime start of topic ===
    Should Contain X Times    ${output.stdout}    [putSample] ${subSystem}::loopTime_${revcode} writing a message containing :    10
    Should Contain X Times    ${output.stdout}    revCode \ : ${revcode}    10
    Should Contain    ${output.stdout}    === ATBuilding_loopTime end of topic ===
...
Read Subscriber
    ...
    Log Many    ${output.stdout}    ${output.stderr}
    Should Contain    ${output.stdout}    ===== ATBuilding subscribers ready =====
    @{full_list}=    Split To Lines    ${output.stdout}    start=1
    ${timestamp_start}=    Get Index From List    ${full_list}    === ATBuilding_timestamp start of topic ===
    ${timestamp_end}=    Get Index From List    ${full_list}    === ATBuilding_timestamp end of topic ===
    ...
    ${loopTime_start}=    Get Index From List    ${full_list}    === ATBuilding_loopTime start of topic ===
    ${loopTime_end}=    Get Index From List    ${full_list}    === ATBuilding_loopTime end of topic ===
