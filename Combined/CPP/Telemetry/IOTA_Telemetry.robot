*** Settings ***
Documentation    IOTA Telemetry communications tests.
...
Start Subscriber
    ...
    Should Contain    ${output}    ===== IOTA subscribers ready =====
...
Start Publisher
    ...
    @{words}=    Split String    ${line}
    ${revcode}=    Set Variable    ${words}[2]
    Should Contain    ${output.stdout}    === IOTA_loopTime start of topic ===
    Should Contain X Times    ${output.stdout}    [putSample] ${subSystem}::loopTime_${revcode} writing a message containing :    10
    Should Contain X Times    ${output.stdout}    revCode \ : ${revcode}    10
    Should Contain    ${output.stdout}    === IOTA_loopTime end of topic ===
...
Read Subscriber
    ...
    Log Many    ${output.stdout}    ${output.stderr}
    Should Contain    ${output.stdout}    ===== IOTA subscribers ready =====
    @{full_list}=    Split To Lines    ${output.stdout}    start=1
    ${timestamp_start}=    Get Index From List    ${full_list}    === IOTA_timestamp start of topic ===
    ${timestamp_end}=    Get Index From List    ${full_list}    === IOTA_timestamp end of topic ===
    ...
    ${loopTime_start}=    Get Index From List    ${full_list}    === IOTA_loopTime start of topic ===
    ${loopTime_end}=    Get Index From List    ${full_list}    === IOTA_loopTime end of topic ===
