*** Settings ***
Documentation    EFDTransformationServer Telemetry communications tests.
Force Tags    cpp    
...
Start Subscriber
    ...
    Should Contain    ${output}    ===== EFDTransformationServer subscribers ready =====
...
Start Publisher
    ...
    Should Contain    ${output.stdout}    === EFDTransformationServer_timestamp start of topic ===
    Should Contain X Times    ${output.stdout}    [putSample] ${subSystem}::timestamp_${revcode} writing a message containing :    10
    Should Contain X Times    ${output.stdout}    revCode \ : ${revcode}    10
    Should Contain    ${output.stdout}    === EFDTransformationServer_timestamp end of topic ===
    ...
    @{words}=    Split String    ${line}
    ${revcode}=    Set Variable    @{words}[2]
    Should Contain    ${output.stdout}    === EFDTransformationServer_loopTime start of topic ===
    Should Contain X Times    ${output.stdout}    [putSample] ${subSystem}::loopTime_${revcode} writing a message containing :    10
    Should Contain X Times    ${output.stdout}    revCode \ : ${revcode}    10
    Should Contain    ${output.stdout}    === EFDTransformationServer_loopTime end of topic ===
...
Read Subscriber
    ...
    Log Many    ${output.stdout}    ${output.stderr}
    Should Contain    ${output.stdout}    ===== EFDTransformationServer subscribers ready =====
