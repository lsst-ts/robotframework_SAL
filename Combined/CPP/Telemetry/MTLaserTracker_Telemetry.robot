*** Settings ***
Documentation    MTLaserTracker Telemetry communications tests.
Force Tags    cpp    
...
Start Subscriber
    ...
    Should Contain    ${string}    ===== MTLaserTracker subscribers ready =====
...
Start Publisher
    ...
    Should Contain    ${output.stdout}    === MTLaserTracker_timestamp start of topic ===
    Should Contain X Times    ${output.stdout}    [putSample] ${subSystem}::timestamp_${revcode} writing a message containing :    10
    Should Contain X Times    ${output.stdout}    revCode \ : ${revcode}    10
    Should Contain    ${output.stdout}    === MTLaserTracker_timestamp end of topic ===
    ...
    @{words}=    Split String    ${line}
    ${revcode}=    Set Variable    @{words}[2]
    Should Contain    ${output.stdout}    === MTLaserTracker_loopTime start of topic ===
    Should Contain X Times    ${output.stdout}    [putSample] ${subSystem}::loopTime_${revcode} writing a message containing :    10
    Should Contain X Times    ${output.stdout}    revCode \ : ${revcode}    10
    Should Contain    ${output.stdout}    === MTLaserTracker_loopTime end of topic ===
...
Read Subscriber
    ...
    Log Many    ${output.stdout}    ${output.stderr}
    Should Contain    ${output.stdout}    ===== MTLaserTracker subscribers ready =====
