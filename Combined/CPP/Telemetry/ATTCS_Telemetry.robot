*** Settings ***
Documentation    ATTCS Telemetry communications tests.
Force Tags    cpp    
...
Start Subscriber
    ...
    Should Contain    ${output}    ===== ATTCS subscribers ready =====
    Sleep    6s
...
Start Publisher
    ...
    @{words}=    Split String    ${line}
    ${revcode}=    Set Variable    @{words}[2]
    Should Contain    ${output.stdout}    === ATTCS_timestamp start of topic ===
    Should Contain X Times    ${output.stdout}    [putSample] ${subSystem}::timestamp_${revcode} writing a message containing :    10
    Should Contain X Times    ${output.stdout}    revCode \ : ${revcode}    10
    Should Contain    ${output.stdout}    === ATTCS_timestamp end of topic ===
...
Read Subscriber
    ...
    Log Many    ${output.stdout}    ${output.stderr}
    Should Contain    ${output.stdout}    ===== ATTCS subscribers ready =====
    @{full_list}=    Split To Lines    ${output.stdout}    start=1
    ${loopTime_start}=    Get Index From List    ${full_list}    === ATTCS_loopTime start of topic ===
    ${loopTime_end}=    Get Index From List    ${full_list}    === ATTCS_loopTime end of topic ===
    ...
    ${timestamp_start}=    Get Index From List    ${full_list}    === ATTCS_timestamp start of topic ===
    ${timestamp_end}=    Get Index From List    ${full_list}    === ATTCS_timestamp end of topic ===
