*** Settings ***
Documentation    MTDome Telemetry communications tests.
...
Start Subscriber
    ...
    Should Contain    ${output}    ===== MTDome subscribers ready =====
...
Start Publisher
    ...
    @{words}=    Split String    ${line}
    ${revcode}=    Set Variable    ${words}[2]
    Should Contain    ${output.stdout}    === MTDome_lightWindScreen start of topic ===
    Should Contain X Times    ${output.stdout}    [putSample] ${subSystem}::lightWindScreen_${revcode} writing a message containing :    10
    Should Contain X Times    ${output.stdout}    revCode \ : ${revcode}    10
    Should Contain    ${output.stdout}    === MTDome_lightWindScreen end of topic ===
    ...
    @{words}=    Split String    ${line}
    ${revcode}=    Set Variable    ${words}[2]
    Should Contain    ${output.stdout}    === MTDome_apertureShutter start of topic ===
    Should Contain X Times    ${output.stdout}    [putSample] ${subSystem}::apertureShutter_${revcode} writing a message containing :    10
    Should Contain X Times    ${output.stdout}    revCode \ : ${revcode}    10
    Should Contain    ${output.stdout}    === MTDome_apertureShutter end of topic ===
    ...
    @{words}=    Split String    ${line}
    ${revcode}=    Set Variable    ${words}[2]
    Should Contain    ${output.stdout}    === MTDome_louvers start of topic ===
    Should Contain X Times    ${output.stdout}    [putSample] ${subSystem}::louvers_${revcode} writing a message containing :    10
    Should Contain X Times    ${output.stdout}    revCode \ : ${revcode}    10
    Should Contain    ${output.stdout}    === MTDome_louvers end of topic ===
    ...
    @{words}=    Split String    ${line}
    ${revcode}=    Set Variable    ${words}[2]
    Should Contain    ${output.stdout}    === MTDome_interlocks start of topic ===
    Should Contain X Times    ${output.stdout}    [putSample] ${subSystem}::interlocks_${revcode} writing a message containing :    10
    Should Contain X Times    ${output.stdout}    revCode \ : ${revcode}    10
    Should Contain    ${output.stdout}    === MTDome_interlocks end of topic ===
    ...
    @{words}=    Split String    ${line}
    ${revcode}=    Set Variable    ${words}[2]
    Should Contain    ${output.stdout}    === MTDome_thermal start of topic ===
    Should Contain X Times    ${output.stdout}    [putSample] ${subSystem}::thermal_${revcode} writing a message containing :    10
    Should Contain X Times    ${output.stdout}    revCode \ : ${revcode}    10
    Should Contain    ${output.stdout}    === MTDome_thermal end of topic ===
...
Read Subscriber
    ...
    Log Many    ${output.stdout}    ${output.stderr}
    Should Contain    ${output.stdout}    ===== MTDome subscribers ready =====
