*** Settings ***
Documentation    EAS Telemetry communications tests.
Force Tags    cpp    
...
Start Subscriber
    ...
    Should Contain    ${output}    ===== EAS subscribers ready =====
...
Start Publisher
    ...
    @{words}=    Split String    ${line}
    ${revcode}=    Set Variable    @{words}[2]
    Should Contain    ${output.stdout}    === EAS_loopTime start of topic ===
    Should Contain X Times    ${output.stdout}    [putSample] ${subSystem}::loopTime_${revcode} writing a message containing :    10
    Should Contain X Times    ${output.stdout}    revCode \ : ${revcode}    10
    Should Contain    ${output.stdout}    === EAS_loopTime end of topic ===
...
Read Subscriber
    ...
    Log Many    ${output.stdout}    ${output.stderr}
    Should Contain    ${output.stdout}    ===== EAS subscribers ready =====
    @{full_list}=    Split To Lines    ${output.stdout}    start=1
    ${timestamp_start}=    Get Index From List    ${full_list}    === EAS_timestamp start of topic ===
    ${timestamp_end}=    Get Index From List    ${full_list}    === EAS_timestamp end of topic ===
    ...
    ${loopTime_start}=    Get Index From List    ${full_list}    === EAS_loopTime start of topic ===
    ${loopTime_end}=    Get Index From List    ${full_list}    === EAS_loopTime end of topic ===
