*** Settings ***
Documentation    Sequencer Telemetry communications tests.
Force Tags    cpp    
...
Start Subscriber
    ...
    Should Contain    ${output}    ===== Sequencer subscribers ready =====
    Sleep    6s
...
Start Publisher
    ...
    Should Contain    ${output.stdout}    === Sequencer_sequencerHeartbeat start of topic ===
    ...
    Should Contain    ${output.stdout}    === Sequencer_sequencerHeartbeat end of topic ===
...
Read Subscriber
    ...
    Log Many    ${output.stdout}    ${output.stderr}
    Should Contain    ${output.stdout}    ===== Sequencer subscribers ready =====
    @{full_list}=    Split To Lines    ${output.stdout}    start=1
    ${sequencerHeartbeat_start}=    Get Index From List    ${full_list}    === Sequencer_sequencerHeartbeat start of topic ===
    ${sequencerHeartbeat_end}=    Get Index From List    ${full_list}    === Sequencer_sequencerHeartbeat end of topic ===
