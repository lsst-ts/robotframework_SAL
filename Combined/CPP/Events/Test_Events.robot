...
Start Sender
    ...
    ${line}=    Grep File    ${SALWorkDir}/avro-templates/${subSystem}/${subSystem}_hash_table.json    "logevent_scalars"
    ...
    Should Contain X Times    ${output.stdout}    === [putSample] ${subSystem}.logevent_scalars writing a message containing :    1
    Should Contain    ${output.stdout}    revCode \ : ${revcode}
    Should Contain    ${output.stdout}    === Event scalars generated =
    ${line}=    Grep File    ${SALWorkDir}/avro-templates/${subSystem}/${subSystem}_hash_table.json    "logevent_arrays"
    ${line}=    Remove String    ${line}    \"    \:    \,
    @{words}=    Split String    ${line}
    ${revcode}=    Set Variable    ${words}[1]
    Comment    ======= Verify ${subSystem}_arrays test messages =======
    Should Contain X Times    ${output.stdout}    === Event arrays iseq = 0    1
    Should Contain X Times    ${output.stdout}    === [putSample] ${subSystem}.logevent_arrays writing a message containing :    1
    Should Contain    ${output.stdout}    revCode \ : ${revcode}
...
Read Logger
    ...
    Should Contain X Times    ${scalars_list}    ${SPACE}${SPACE}${SPACE}${SPACE}byte0 : 1    1
