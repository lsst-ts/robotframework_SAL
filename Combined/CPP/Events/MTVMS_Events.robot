...
Start Sender
    ...
    Comment    ======= Verify ${subSystem}_acquisitionRate test messages =======
    ${line}=    Grep File    ${SALWorkDir}/idl-templates/validated/${subSystem}_revCodes.tcl    ${subSystem}_logevent_acquisitionRate
    @{words}=    Split String    ${line}
    ${revcode}=    Set Variable    @{words}[2]
    Should Contain X Times    ${output.stdout}    === Event acquisitionRate iseq = 0    1
    Should Contain X Times    ${output.stdout}    === [putSample] ${subSystem}::logevent_acquisitionRate_${revcode} writing a message containing :    1
    Should Contain    ${output.stdout}    revCode \ : ${revcode}    10
    Should Contain    ${output.stdout}    === Event acquisitionRate generated =
...
Read Logger
    ...
    Should Contain X Times    ${settingsApplied_list}    ${SPACE}${SPACE}${SPACE}${SPACE}timestamp : 1    1
    ...
    ${acquisitionRate_start}=    Get Index From List    ${full_list}    === Event acquisitionRate received =${SPACE}
    ${end}=    Get Index From List    ${full_list}    ${SPACE}${SPACE}${SPACE}${SPACE}priority : 1    start=${acquisitionRate_start}
    ${acquisitionRate_end}=    Evaluate    ${end}+${1}
    ${acquisitionRate_list}=    Get Slice From List    ${full_list}    start=${acquisitionRate_start}    end=${acquisitionRate_end}
    Should Contain X Times    ${acquisitionRate_list}    ${SPACE}${SPACE}${SPACE}${SPACE}timestamp : 1    1
    Should Contain X Times    ${acquisitionRate_list}    ${SPACE}${SPACE}${SPACE}${SPACE}rateInHz : 1    1
    Should Contain X Times    ${acquisitionRate_list}    ${SPACE}${SPACE}${SPACE}${SPACE}priority : 1    1
