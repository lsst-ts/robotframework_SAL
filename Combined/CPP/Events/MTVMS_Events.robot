...
Start Sender
    ...
    ${line}=    Grep File    ${SALWorkDir}/avro-templates/${subSystem}/${subSystem}_hash_table.json    "logevent_fpgaState"
    ...
    Should Contain X Times    ${output.stdout}    === [putSample] ${subSystem}.logevent_fpgaState writing a message containing :    1
    Should Contain    ${output.stdout}    revCode \ : ${revcode}
    Should Contain    ${output.stdout}    === Event fpgaState generated =
    ${line}=    Grep File    ${SALWorkDir}/avro-templates/${subSystem}/${subSystem}_hash_table.json    "logevent_timeSynchronization"
    ${line}=    Remove String    ${line}    \"    \:    \,
    @{words}=    Split String    ${line}
    ${revcode}=    Set Variable    ${words}[1]
    ...
    Should Contain X Times    ${output.stdout}    === [putSample] ${subSystem}.logevent_timeSynchronization writing a message containing :    1
    Should Contain    ${output.stdout}    revCode \ : ${revcode}
...
Read Logger
    ...
    ${end}=    Evaluate    ${fpgaState_start}+${8}
    ...
    ${end}=    Evaluate    ${timeSynchronization_start}+${4}
