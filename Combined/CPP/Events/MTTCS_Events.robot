...
Start Sender
    ...
    Comment    ======= Verify ${subSystem}_wfpDataReady test messages =======
    ${line}=    Grep File    ${SALWorkDir}/idl-templates/validated/${subSystem}_revCodes.tcl    ${subSystem}_logevent_wfpDataReady
    @{words}=    Split String    ${line}
    ${revcode}=    Set Variable    @{words}[2]
    Should Contain X Times    ${output.stdout}    === [putSample] ${subSystem}::logevent_wfpDataReady_${revcode} writing a message containing :    1
    Should Contain    ${output.stdout}    revCode \ : ${revcode}    10
    Should Contain    ${output.stdout}    === ${subSystem}_wfpDataReady end of topic ===
    Comment    ======= Verify ${subSystem}_zemaxError test messages =======
    ${line}=    Grep File    ${SALWorkDir}/idl-templates/validated/${subSystem}_revCodes.tcl    ${subSystem}_logevent_zemaxError
    @{words}=    Split String    ${line}
    ${revcode}=    Set Variable    @{words}[2]
    Should Contain X Times    ${output.stdout}    === [putSample] ${subSystem}::logevent_zemaxError_${revcode} writing a message containing :    1
    Should Contain    ${output.stdout}    revCode \ : ${revcode}    10
    Should Contain    ${output.stdout}    === ${subSystem}_zemaxError end of topic ===
    ...
    Comment    ======= Verify ${subSystem}_heartbeatIn test messages =======
    ${line}=    Grep File    ${SALWorkDir}/idl-templates/validated/${subSystem}_revCodes.tcl    ${subSystem}_logevent_heartbeatIn
    @{words}=    Split String    ${line}
    ${revcode}=    Set Variable    @{words}[2]
    Should Contain X Times    ${output.stdout}    === [putSample] ${subSystem}::logevent_heartbeatIn_${revcode} writing a message containing :    1
    Should Contain    ${output.stdout}    revCode \ : ${revcode}    10
    Should Contain    ${output.stdout}    === ${subSystem}_heartbeatIn end of topic ===
    Comment    ======= Verify ${subSystem}_filterChangeInPosition test messages =======
    ${line}=    Grep File    ${SALWorkDir}/idl-templates/validated/${subSystem}_revCodes.tcl    ${subSystem}_logevent_filterChangeInPosition
    @{words}=    Split String    ${line}
    ${revcode}=    Set Variable    @{words}[2]
    Should Contain X Times    ${output.stdout}    === [putSample] ${subSystem}::logevent_filterChangeInPosition_${revcode} writing a message containing :    1
    Should Contain    ${output.stdout}    revCode \ : ${revcode}    10
    Should Contain    ${output.stdout}    === ${subSystem}_filterChangeInPosition end of topic ===
    Comment    ======= Verify ${subSystem}_targetInPosition test messages =======
    ${line}=    Grep File    ${SALWorkDir}/idl-templates/validated/${subSystem}_revCodes.tcl    ${subSystem}_logevent_targetInPosition
    @{words}=    Split String    ${line}
    ${revcode}=    Set Variable    @{words}[2]
    Should Contain X Times    ${output.stdout}    === [putSample] ${subSystem}::logevent_targetInPosition_${revcode} writing a message containing :    1
    Should Contain    ${output.stdout}    revCode \ : ${revcode}    10
    Should Contain    ${output.stdout}    === ${subSystem}_targetInPosition end of topic ===
...
Read Logger
    ...
    ${wfpDataReady_start}=    Get Index From List    ${full_list}    === ${subSystem}_wfpDataReady start of topic ===
    ${wfpDataReady_end}=    Get Index From List    ${full_list}    === ${subSystem}_wfpDataReady end of topic ===
    ${wfpDataReady_list}=    Get Slice From List    ${full_list}    start=${wfpDataReady_start}    end=${wfpDataReady_end}
    Should Contain X Times    ${wfpDataReady_list}    ${SPACE}${SPACE}${SPACE}${SPACE}imagesLocationUri : LSST    1
    Should Contain X Times    ${wfpDataReady_list}    ${SPACE}${SPACE}${SPACE}${SPACE}imagesMatchUri : LSST    1
    Should Contain X Times    ${wfpDataReady_list}    ${SPACE}${SPACE}${SPACE}${SPACE}priority : 1    1
    ${zemaxError_start}=    Get Index From List    ${full_list}    === ${subSystem}_zemaxError start of topic ===
    ${zemaxError_end}=    Get Index From List    ${full_list}    === ${subSystem}_zemaxError end of topic ===
    ${zemaxError_list}=    Get Slice From List    ${full_list}    start=${zemaxError_start}    end=${zemaxError_end}
    Should Contain X Times    ${zemaxError_list}    ${SPACE}${SPACE}${SPACE}${SPACE}details : LSST    1
    Should Contain X Times    ${zemaxError_list}    ${SPACE}${SPACE}${SPACE}${SPACE}priority : 1    1
    ...
    ${heartbeatIn_start}=    Get Index From List    ${full_list}    === ${subSystem}_heartbeatIn start of topic ===
    ${heartbeatIn_end}=    Get Index From List    ${full_list}    === ${subSystem}_heartbeatIn end of topic ===
    ${heartbeatIn_list}=    Get Slice From List    ${full_list}    start=${heartbeatIn_start}    end=${heartbeatIn_end}
    Should Contain X Times    ${heartbeatIn_list}    ${SPACE}${SPACE}${SPACE}${SPACE}heartbeatIn : 1    1
    Should Contain X Times    ${heartbeatIn_list}    ${SPACE}${SPACE}${SPACE}${SPACE}priority : 1    1
    ${filterChangeInPosition_start}=    Get Index From List    ${full_list}    === ${subSystem}_filterChangeInPosition start of topic ===
    ${filterChangeInPosition_end}=    Get Index From List    ${full_list}    === ${subSystem}_filterChangeInPosition end of topic ===
    ${filterChangeInPosition_list}=    Get Slice From List    ${full_list}    start=${filterChangeInPosition_start}    end=${filterChangeInPosition_end}
    Should Contain X Times    ${filterChangeInPosition_list}    ${SPACE}${SPACE}${SPACE}${SPACE}filterChangeInPosition : 1    1
    Should Contain X Times    ${filterChangeInPosition_list}    ${SPACE}${SPACE}${SPACE}${SPACE}priority : 1    1
    ${targetInPosition_start}=    Get Index From List    ${full_list}    === ${subSystem}_targetInPosition start of topic ===
    ${targetInPosition_end}=    Get Index From List    ${full_list}    === ${subSystem}_targetInPosition end of topic ===
    ${targetInPosition_list}=    Get Slice From List    ${full_list}    start=${targetInPosition_start}    end=${targetInPosition_end}
    Should Contain X Times    ${targetInPosition_list}    ${SPACE}${SPACE}${SPACE}${SPACE}targetInPosition : 1    1
    Should Contain X Times    ${targetInPosition_list}    ${SPACE}${SPACE}${SPACE}${SPACE}priority : 1    1
