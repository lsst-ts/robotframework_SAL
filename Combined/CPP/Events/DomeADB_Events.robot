*** Settings ***
Documentation    DomeADB_Events communications tests.
Force Tags    cpp    
...
Start Sender
    ...
    Comment    ======= Verify ${subSystem}_driveDriveFault test messages =======
    ${line}=    Grep File    ${SALWorkDir}/idl-templates/validated/${subSystem}_revCodes.tcl    ${subSystem}_logevent_driveDriveFault
    @{words}=    Split String    ${line}
    ${revcode}=    Set Variable    @{words}[2]
    Should Contain X Times    ${output.stdout}    === [putSample] ${subSystem}::logevent_driveDriveFault_${revcode} writing a message containing :    1
    Should Contain    ${output.stdout}    revCode \ : ${revcode}    10
    Should Contain    ${output.stdout}    === ${subSystem}_driveDriveFault end of topic ===
    Comment    ======= Verify ${subSystem}_rotationEnabled test messages =======
    ${line}=    Grep File    ${SALWorkDir}/idl-templates/validated/${subSystem}_revCodes.tcl    ${subSystem}_logevent_rotationEnabled
    @{words}=    Split String    ${line}
    ${revcode}=    Set Variable    @{words}[2]
    Should Contain X Times    ${output.stdout}    === [putSample] ${subSystem}::logevent_rotationEnabled_${revcode} writing a message containing :    1
    Should Contain    ${output.stdout}    revCode \ : ${revcode}    10
    Should Contain    ${output.stdout}    === ${subSystem}_rotationEnabled end of topic ===
    Comment    ======= Verify ${subSystem}_rotationPrevented test messages =======
    ${line}=    Grep File    ${SALWorkDir}/idl-templates/validated/${subSystem}_revCodes.tcl    ${subSystem}_logevent_rotationPrevented
    @{words}=    Split String    ${line}
    ${revcode}=    Set Variable    @{words}[2]
    Should Contain X Times    ${output.stdout}    === [putSample] ${subSystem}::logevent_rotationPrevented_${revcode} writing a message containing :    1
    Should Contain    ${output.stdout}    revCode \ : ${revcode}    10
    Should Contain    ${output.stdout}    === ${subSystem}_rotationPrevented end of topic ===
    ...
    Comment    ======= Verify ${subSystem}_accelerationLimitReached test messages =======
    ${line}=    Grep File    ${SALWorkDir}/idl-templates/validated/${subSystem}_revCodes.tcl    ${subSystem}_logevent_accelerationLimitReached
    @{words}=    Split String    ${line}
    ${revcode}=    Set Variable    @{words}[2]
    Should Contain X Times    ${output.stdout}    === [putSample] ${subSystem}::logevent_accelerationLimitReached_${revcode} writing a message containing :    1
    Should Contain    ${output.stdout}    revCode \ : ${revcode}    10
    Should Contain    ${output.stdout}    === ${subSystem}_accelerationLimitReached end of topic ===
    Comment    ======= Verify ${subSystem}_speedLimitSubsided test messages =======
    ${line}=    Grep File    ${SALWorkDir}/idl-templates/validated/${subSystem}_revCodes.tcl    ${subSystem}_logevent_speedLimitSubsided
    @{words}=    Split String    ${line}
    ${revcode}=    Set Variable    @{words}[2]
    Should Contain X Times    ${output.stdout}    === [putSample] ${subSystem}::logevent_speedLimitSubsided_${revcode} writing a message containing :    1
    Should Contain    ${output.stdout}    revCode \ : ${revcode}    10
    Should Contain    ${output.stdout}    === ${subSystem}_speedLimitSubsided end of topic ===
    Comment    ======= Verify ${subSystem}_accelerationLimitSubsided test messages =======
    ${line}=    Grep File    ${SALWorkDir}/idl-templates/validated/${subSystem}_revCodes.tcl    ${subSystem}_logevent_accelerationLimitSubsided
    @{words}=    Split String    ${line}
    ${revcode}=    Set Variable    @{words}[2]
    Should Contain X Times    ${output.stdout}    === [putSample] ${subSystem}::logevent_accelerationLimitSubsided_${revcode} writing a message containing :    1
    Should Contain    ${output.stdout}    revCode \ : ${revcode}    10
    Should Contain    ${output.stdout}    === ${subSystem}_accelerationLimitSubsided end of topic ===
    ...
    Comment    ======= Verify ${subSystem}_motionModeChanged test messages =======
    ${line}=    Grep File    ${SALWorkDir}/idl-templates/validated/${subSystem}_revCodes.tcl    ${subSystem}_logevent_motionModeChanged
    @{words}=    Split String    ${line}
    ${revcode}=    Set Variable    @{words}[2]
    Should Contain X Times    ${output.stdout}    === [putSample] ${subSystem}::logevent_motionModeChanged_${revcode} writing a message containing :    1
    Should Contain    ${output.stdout}    revCode \ : ${revcode}    10
    Should Contain    ${output.stdout}    === ${subSystem}_motionModeChanged end of topic ===
...
Read Logger
    ...
    ${driveDriveFault_start}=    Get Index From List    ${full_list}    === ${subSystem}_driveDriveFault start of topic ===
    ${driveDriveFault_end}=    Get Index From List    ${full_list}    === ${subSystem}_driveDriveFault end of topic ===
    ${driveDriveFault_list}=    Get Slice From List    ${full_list}    start=${driveDriveFault_start}    end=${driveDriveFault_end}
    Should Contain X Times    ${driveDriveFault_list}    ${SPACE}${SPACE}${SPACE}${SPACE}driveId : 1    1
    Should Contain X Times    ${driveDriveFault_list}    ${SPACE}${SPACE}${SPACE}${SPACE}errorCode : LSST    1
    Should Contain X Times    ${driveDriveFault_list}    ${SPACE}${SPACE}${SPACE}${SPACE}priority : 1    1
    ${rotationEnabled_start}=    Get Index From List    ${full_list}    === ${subSystem}_rotationEnabled start of topic ===
    ${rotationEnabled_end}=    Get Index From List    ${full_list}    === ${subSystem}_rotationEnabled end of topic ===
    ${rotationEnabled_list}=    Get Slice From List    ${full_list}    start=${rotationEnabled_start}    end=${rotationEnabled_end}
    Should Contain X Times    ${rotationEnabled_list}    ${SPACE}${SPACE}${SPACE}${SPACE}status : 1    1
    Should Contain X Times    ${rotationEnabled_list}    ${SPACE}${SPACE}${SPACE}${SPACE}priority : 1    1
    ${rotationPrevented_start}=    Get Index From List    ${full_list}    === ${subSystem}_rotationPrevented start of topic ===
    ${rotationPrevented_end}=    Get Index From List    ${full_list}    === ${subSystem}_rotationPrevented end of topic ===
    ${rotationPrevented_list}=    Get Slice From List    ${full_list}    start=${rotationPrevented_start}    end=${rotationPrevented_end}
    Should Contain X Times    ${rotationPrevented_list}    ${SPACE}${SPACE}${SPACE}${SPACE}causeId : LSST    1
    Should Contain X Times    ${rotationPrevented_list}    ${SPACE}${SPACE}${SPACE}${SPACE}priority : 1    1
    ...
    Should Contain X Times    ${lockingPinEngaged_list}    ${SPACE}${SPACE}${SPACE}${SPACE}status : 1    1
    ...
    Should Contain X Times    ${lockingPinDisengaged_list}    ${SPACE}${SPACE}${SPACE}${SPACE}status : 1    1
    ...
    Should Contain X Times    ${speedLimitReached_list}    ${SPACE}${SPACE}${SPACE}${SPACE}status : 1    1
    Should Contain X Times    ${speedLimitReached_list}    ${SPACE}${SPACE}${SPACE}${SPACE}priority : 1    1
    ${accelerationLimitReached_start}=    Get Index From List    ${full_list}    === ${subSystem}_accelerationLimitReached start of topic ===
    ${accelerationLimitReached_end}=    Get Index From List    ${full_list}    === ${subSystem}_accelerationLimitReached end of topic ===
    ${accelerationLimitReached_list}=    Get Slice From List    ${full_list}    start=${accelerationLimitReached_start}    end=${accelerationLimitReached_end}
    Should Contain X Times    ${accelerationLimitReached_list}    ${SPACE}${SPACE}${SPACE}${SPACE}status : 1    1
    Should Contain X Times    ${accelerationLimitReached_list}    ${SPACE}${SPACE}${SPACE}${SPACE}priority : 1    1
    ${speedLimitSubsided_start}=    Get Index From List    ${full_list}    === ${subSystem}_speedLimitSubsided start of topic ===
    ${speedLimitSubsided_end}=    Get Index From List    ${full_list}    === ${subSystem}_speedLimitSubsided end of topic ===
    ${speedLimitSubsided_list}=    Get Slice From List    ${full_list}    start=${speedLimitSubsided_start}    end=${speedLimitSubsided_end}
    Should Contain X Times    ${speedLimitSubsided_list}    ${SPACE}${SPACE}${SPACE}${SPACE}status : 1    1
    Should Contain X Times    ${speedLimitSubsided_list}    ${SPACE}${SPACE}${SPACE}${SPACE}priority : 1    1
    ${accelerationLimitSubsided_start}=    Get Index From List    ${full_list}    === ${subSystem}_accelerationLimitSubsided start of topic ===
    ${accelerationLimitSubsided_end}=    Get Index From List    ${full_list}    === ${subSystem}_accelerationLimitSubsided end of topic ===
    ${accelerationLimitSubsided_list}=    Get Slice From List    ${full_list}    start=${accelerationLimitSubsided_start}    end=${accelerationLimitSubsided_end}
    Should Contain X Times    ${accelerationLimitSubsided_list}    ${SPACE}${SPACE}${SPACE}${SPACE}status : 1    1
    Should Contain X Times    ${accelerationLimitSubsided_list}    ${SPACE}${SPACE}${SPACE}${SPACE}priority : 1    1
    ...
    ${motionModeChanged_start}=    Get Index From List    ${full_list}    === ${subSystem}_motionModeChanged start of topic ===
    ${motionModeChanged_end}=    Get Index From List    ${full_list}    === ${subSystem}_motionModeChanged end of topic ===
    ${motionModeChanged_list}=    Get Slice From List    ${full_list}    start=${motionModeChanged_start}    end=${motionModeChanged_end}
    Should Contain X Times    ${motionModeChanged_list}    ${SPACE}${SPACE}${SPACE}${SPACE}newMode : 1    1
    Should Contain X Times    ${motionModeChanged_list}    ${SPACE}${SPACE}${SPACE}${SPACE}priority : 1    1
