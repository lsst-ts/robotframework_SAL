...
Start Sender
    ...
    Should Contain    ${output.stdout}    === ${subSystem}_appliedSetpoint end of topic ===
...
Read Logger
    ...
    ${appliedSetpoint_start}=    Get Index From List    ${full_list}    === ${subSystem}_appliedSetpoint start of topic ===
    ${appliedSetpoint_end}=    Get Index From List    ${full_list}    === ${subSystem}_appliedSetpoint end of topic ===
    ${appliedSetpoint_list}=    Get Slice From List    ${full_list}    start=${appliedSetpoint_start}    end=${appliedSetpoint_end}
    Should Contain X Times    ${appliedSetpoint_list}    ${SPACE}${SPACE}${SPACE}${SPACE}setpoint : 1    1
    Should Contain X Times    ${appliedSetpoint_list}    ${SPACE}${SPACE}${SPACE}${SPACE}priority : 1    1
