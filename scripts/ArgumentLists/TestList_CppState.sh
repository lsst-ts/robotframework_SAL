...
# Define filename
argfile=$HOME/trunk/robotframework_SAL/CppStateMachine_Tests.list
...
echo "--name CppStateMachine" >> $argfile
echo "--output CppStateMachine" >> $argfile
echo "" >> $argfile
echo "# List of test suites" >> $argfile

# Find test suites and add them to the argument file.
ls -1 $HOME/trunk/robotframework_SAL/CPP/StateMachine |sed 's/^/CPP\/StateMachine\//' >> $argfile
