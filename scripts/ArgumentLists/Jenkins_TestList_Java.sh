...
# Define filename
argfile=$ROBOT_FRAMEWORK_REPO_DIR/Jenkins_Java.list
...
basename `ls -1 $ROBOT_FRAMEWORK_REPO_DIR/JAVA/Telemetry/Camera_*` |sed 's/^/rbtsal\/JAVA\/Telemetry\//' >> $argfile
echo "" >> $argfile
echo "# JAVA State Machine test suites" >> $argfile
basename `ls -1 $ROBOT_FRAMEWORK_REPO_DIR/JAVA/StateMachine/Camera_*` |sed 's/^/rbtsal\/JAVA\/StateMachine\//' >> $argfile
echo "" >> $argfile
echo "# JAVA Commander/Controller test suites" >> $argfile
basename `ls -1 $ROBOT_FRAMEWORK_REPO_DIR/JAVA/Commands/Camera_*` |sed 's/^/rbtsal\/JAVA\/Commands\//' >> $argfile
echo "" >> $argfile
echo "# JAVA Event Sender/Logger test suites" >> $argfile
basename `ls -1 $ROBOT_FRAMEWORK_REPO_DIR/JAVA/Events/Camera_*` |sed 's/^/rbtsal\/JAVA\/Events\//' >> $argfile
