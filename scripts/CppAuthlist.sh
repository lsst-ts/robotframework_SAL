...
workDir=$ROBOTFRAMEWORK_SAL_DIR/AuthList/CPP
...
function main() {
    ...
    if [[ "$rtlang" =~ "cpp" ]]; then
        ...
        echo Skipping: $subSystem has no C++ AuthList topics.
        return 0
    fi
}
...
function createSettings {
    ...
    echo "Force Tags    authlist    cpp    $skipped" >> $testSuite
...
function verifyAuthlistTestFiles {
    ...
    echo "    File Should Exist    \${SALWorkDir}/\${subSystem}/cpp/src/sacpp_\${subSystem}_setAuthList_commander" >> $testSuite
    echo "    File Should Exist    \${SALWorkDir}/\${subSystem}/cpp/src/sacpp_\${subSystem}_setAuthList_controller" >> $testSuite
    echo "    File Should Exist    \${SALWorkDir}/\${subSystem}/cpp/src/sacpp_\${subSystem}_setLogLevel_commander" >> $testSuite
    echo "    File Should Exist    \${SALWorkDir}/\${subSystem}/cpp/src/sacpp_\${subSystem}_setLogLevel_controller" >> $testSuite
...
function startAuthlistTestScript {
    ...
    echo "    \${output}=    Run Process    \${SALWorkDir}/\${subSystem}/cpp/src/testAuthList.sh    alias=\${subSystem}_authlist    shell=True    stdout=\${EXECDIR}\${/}\${subSystem}_stdout.txt    stderr=\${EXECDIR}\${/}\${subSystem}_stderr.txt" >> $testSuite
    echo "    Log Many    \${output.stdout}    \${output.stderr}" >> $testSuite
    echo "    Comment    ======= Verify \${subSystem} AuthList test messages =======" >> $testSuite
    echo "    Should Contain    \${output.stdout}    Starting sacpp_\${subSystem}_setLogLevel_controller" >> $testSuite
...
function verifyFirstTest {
    ...
    echo "    Should Contain    \${test_output}    === [issueCommand_setAuthList] writing a command containing :" >> $testSuite
    ...
    echo "    Should Contain    \${test_output}    === command setAuthList issued =\${SPACE}" >> $testSuite
    ...
    echo "    Should Contain    \${test_output}    === [issueCommand_setLogLevel] writing a command containing :" >> $testSuite
    echo "    Should Contain    \${test_output}    === command setLogLevel issued =\${SPACE}" >> $testSuite
    echo "    Should Contain    \${test_output}    === command setLogLevel received =\${SPACE}" >> $testSuite
    ...
    echo "    Should Contain X Times    \${test_output}    \${SPACE}\${SPACE}\${SPACE}\${SPACE}identity\${SPACE}\${SPACE}\${SPACE}\${SPACE}: ${identity}    3" >> $testSuite
    echo "    Should Match Regexp    \${test_output}[-1]    === \\\[waitForCompletion_setLogLevel\\\] command [0-9]{10,} completed ok :" >> $testSuite
    echo "" >> $testSuite
}
...
function verifyThirdTest {
    ...
    echo "    Should Contain    \${test_output}    === [ackCommand_setLogLevel] acknowledging a command with :" >> $testSuite
    echo "    Should Contain    \${test_output}    \${SPACE}\${SPACE}\${SPACE}\${SPACE}result\${SPACE}\${SPACE}\${SPACE}: Done : OK" >> $testSuite
    echo "    Should Contain X Times    \${test_output}    \${SPACE}\${SPACE}\${SPACE}\${SPACE}authorizedUsers : user@host    2" >> $testSuite
    echo "    Should Contain X Times    \${test_output}    \${SPACE}\${SPACE}\${SPACE}\${SPACE}nonAuthorizedCSCs :\${SPACE}    2" >> $testSuite
    echo "    Should Contain X Times    \${test_output}    \${SPACE}\${SPACE}\${SPACE}\${SPACE}identity\${SPACE}\${SPACE}\${SPACE}\${SPACE}: ${identity}    3" >> $testSuite
    echo "    Should Match Regexp    \${test_output}[-1]    === \\\[waitForCompletion_setLogLevel\\\] command [0-9]{10,} completed ok :" >> $testSuite
    echo "" >> $testSuite
}
...
function verifySixthTest {
    ...
    echo "    Should Contain    \${test_output}    === [issueCommand_setLogLevel] writing a command containing :" >> $testSuite
    echo "    Should Contain    \${test_output}    === command setLogLevel issued =\${SPACE}" >> $testSuite
    echo "    Should Contain X Times    \${test_output}    \${SPACE}\${SPACE}\${SPACE}\${SPACE}nonAuthorizedCSCs : Test    2" >> $testSuite
    echo "    Should Contain X Times    \${test_output}    \${SPACE}\${SPACE}\${SPACE}\${SPACE}authorizedUsers : user@host,user2@other    2" >> $testSuite
    echo "    Should Contain X Times    \${test_output}    \${SPACE}\${SPACE}\${SPACE}\${SPACE}identity\${SPACE}\${SPACE}\${SPACE}\${SPACE}: ${identity}    3" >> $testSuite
    echo "    Should Match Regexp    \${test_output}[-1]    === \\\[waitForCompletion_setLogLevel\\\] command [0-9]{10,} completed ok :" >> $testSuite
    echo "" >> $testSuite
...
function createTestSuite {
    ...
    enumeration=$( xmlstarlet sel -t -m "//SALSubsystemSet/SALSubsystem[Name='$subSystem']/IndexEnumeration" -v . -n $TS_XML_DIR/python/lsst/ts/xml/data/sal_interfaces/SALSubsystems.xml )
