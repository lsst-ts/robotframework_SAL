...
function clearTestSuites() {
	...
	files=$(ls -1 $HOME/trunk/robotframework_SAL/Separate/$language/$topic_type/${subsystem}_* ; ls -1 $HOME/trunk/robotframework_SAL/Combined/$language/${subsystem}_${topic_type}*)
    if [ $? -eq 0 ]; then
    	echo "Deleting:"
		echo "$files"
    	rm $files
