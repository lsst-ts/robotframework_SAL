...
function getTelemetryTopics() {
    ...
    local output=$( xml sel -t -m "//SALTelemetrySet/SALTelemetry/EFDB_Topic" -v . -n $TS_XML_REPO_DIR/sal_interfaces/${subSystem}/${subSystem}_Telemetry.xml |sed "s/${subSystem}_//" )
    echo $output
}

function getCommandTopics() {
	local subSystem=$1
    local output=$( xml sel -t -m "//SALCommandSet/SALCommand/EFDB_Topic" -v . -n $TS_XML_REPO_DIR/sal_interfaces/${subSystem}/${subSystem}_Commands.xml |sed "s/${subSystem}_command_//" )
    echo $output
}

function getEventTopics() {
	local subSystem=$1
    local output=$( xml sel -t -m "//SALEventSet/SALEvent/EFDB_Topic" -v . -n $TS_XML_REPO_DIR/sal_interfaces/${subSystem}/${subSystem}_Events.xml |sed "s/${subSystem}_logevent_//" )
    echo $output
}
...
function clearTestSuites() {
	...
	#files=$(ls -1 $ROBOT_FRAMEWORK_REPO_DIR/Separate/$language/$topic_type/${subsystem}_* ; ls -1 $ROBOT_FRAMEWORK_REPO_DIR/Combined/$language/$topic_type/${subsystem}_${topic_type}*)
	files=$(ls -1 $ROBOT_FRAMEWORK_REPO_DIR/Combined/$language/$topic_type/${subsystem}_${topic_type}*)
    if [ $? -eq 0 ]; then
    	echo "Deleting:"
		echo "$files"
    	rm $files
