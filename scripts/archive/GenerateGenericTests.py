	...
	if  csc == "all":
		for csc in csc_array:
			if ('cpp' in language) or ('all' in language):
				subprocess.check_call(["./CppGenericEvents.sh", csc])
			if ('java' in language) or ('all' in language):
				print("Not done yet")
				#subprocess.check_call(["./JavaGenericEvents.sh", csc])
			if ('python' in language) or ('all' in language):
				print("Not done yet")
				subprocess.check_call(["./PythonGenericEvents.sh", csc])
